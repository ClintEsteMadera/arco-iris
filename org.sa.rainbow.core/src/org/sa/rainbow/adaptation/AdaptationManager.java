...
/**
 * The Rainbow Adaptation Engine...
 * Currently implements a learner interface to interact with Nick Lynn's learner.
 * 
 * @author Shang-Wen Cheng (zensoul@cs.cmu.edu)
 */
public final class AdaptationManager extends AbstractRainbowRunnable /*implements IRainbowLearner*/ {
	...
	/** The prefix to be used by the strategy which takes a "leap" by achieving
	 *  a similar adaptation that would have taken multiple increments of the
	 *  non-leap version, but at a potential "cost" in non-dire scenarios.
	 */
	...
	/**
	 * Removes a Strategy from the list of pending strategies, marking it as
	 * being completed (doesn't incorporate outcome).
	...
	public double computeSystemInstantUtility () {
		Map<String,Double> weights = Rainbow.instance().preferenceDesc().weights.get(Rainbow.property(Rainbow.PROPKEY_SCENARIO));
	...
	/**
	 * For JUnit testing, allows fetching the strategy repertoire.
	 * NOT for public use!
	...
	/**
	 * For JUnit testing, allows fetching the utility objects.
	 * NOT for public use!
	...
	/* Algorithm:
	 *   - Iterate through repertoire searching for enabled strategies,
	 *     where "enabled" means applicable to current system condition
	 *     NOTE:  A Strategy is "applicable" iff the conditions of
	 *       applicability of the root tactic is true
	 *     TODO: Need to check if above is good assumption
	 *   - Calculate scores of the enabled strategies
	 *     = this involves evaluating the meta-information of the tactics
	 *       in each strategy
	 *   - Select and execute the highest scoring strategy
	 */
	private void doAdaptation () {
		log("Adaptation triggered, let's begin!");
		if (_stopWatchForTesting != null) _stopWatchForTesting.start();
		...
		for (Stitch stitch : m_repertoire) {
			if (! stitch.script.isApplicableForModel((IAcmeModel )m_model.getAcmeModel())) {
				if (m_logger.isDebugEnabled()) m_logger.debug("x. skipping " + stitch.script.getName());
		...
		log(">> repertoire: " + appSubsetByName.size() + " / " + availCnt
				+ " strateg" + (availCnt > 1 ? "ies" : "y"));
		...
		if (_stopWatchForTesting != null) _stopWatchForTesting.stop();
	 ...
	 * and use the aggregate values plus stakeholder utility preferences to
	 * compute an integer score for each Strategy, between 0 and 100.
	...
	private SortedMap<Double,Strategy> scoreStrategies (Map<String,Strategy> subset) {
		...
		for (Strategy strategy : subset.values()) {
			...
			if (conds == null) conds = new double[aggAtt.size()];
			if (condsPred == null) condsPred = new double[aggAtt.size()];
			int i = 0;
			double score = 0.0;
		...
		if (predictionEnabled) log("condP! : " + Arrays.toString(condsPred));
		return scored;
	}

	/**
	 * Retrieves the adaptation repertoire; for each tactic, store the
	 * respective tactic attribute vectors.
	 */
	private void initAdaptationRepertoire () {
		File stitchPath = Util.getRelativeToPath(Rainbow.instance().getTargetPath(), Rainbow.property(Rainbow.PROPKEY_SCRIPT_PATH));
	...
	private void defineAttributes (Stitch stitch, Map<String,Map<String,Object>> attrVectorMap) {
		for (Tactic t : stitch.script.tactics) {
			Map<String,Object> attributes = (Map<String,Object> )attrVectorMap.get(t.getName());
			if (attributes != null) {
