...
\label{sec:casosPracticos}
	...
	\subsection{Caso 6: Comportamiento Ante Los Cambios en el Entorno de Ejecución}
		...
		Este caso de prueba pretende demostrar cómo Arco Iris varía su comportamiento dependiendo el entorno bajo el cual se
		...
		variaciones:
		\begin{itemize}
		  \item Un escenario de prioridad 3 cuya definición acota el tiempo de respuesta a 600 ms. cuando el sistema se
		  encuentra en carga normal.
		  \item Un escenario de prioridad 1 que define que el tiempo de respuesta no debe superar los 900 ms. cuando el
		  sistema se encuentra bajo alta carga.
		  \item Un escenario de prioridad 2 que predica sobre el costo de servidores, limitando al sistema a utilizar como
		  máximo un servidor cuando se encuentre en un entorno de carga normal.
		\end{itemize}
		
		También ha sido necesario modificar el comportamiento del sistema utilizado en las pruebas anteriores para lograr que
		el entorno pase a ser de carga alta en lugar de normal. 
		
		Los escenarios de tiempo de respuesta tendrán asociadas como estrategia de auto reparación a
		\verb@EnlistServerResponseTime@, utilizada en pruebas anteriores, mientras que el escenario de costo no poseerá
		estrategias asociadas.
		
		Es importante remarcar que, a diferencia de los casos anteriores, cuando el sistema se encuentra bajo carga normal
		(i.e por debajo del umbral de 800 ms), la prioridad del escenario de costo es superior a la del escenario de tiempo
		de respuesta. Esto hace que Arco Iris no tome acción alguna mientras el sistema se mantenga en este entorno.
		
		Luego de superados los 800 ms. el sistema pasa a estar en alta carga, por lo cual dos de los tres escenarios pasan a
		satisfacerse trivialmente, entonces Arco Iris deberá solamente procurar que se cumpla el escenario de tiempo de
		respuesta que impide que se superen los 900 ms. Puede observarse en la figura \ref{fig:testcase6_expRespTime_cost} que
		la auto reparación decide agregar un servidor aproximadamente a partir de los 23 segundos de iniciada la simulación,		reparando efectivamente el escenario en cuestión.
		\begin{figure}[H]
			\centering
				\includegraphics[width=1.0\textwidth]{images/testcase6_expRespTime_cost.png}
			\caption{Comportamiento del sistema bajo distintos entornos}
