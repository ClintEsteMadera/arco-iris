...
\label{sec:casosPracticos}
	...
	\label{sec:configBasicaCasosPrueba}
		...
		Antes de comenzar, cabe mencionar que los \emph{logs} generados por Arco Iris ofrecen la información necesaria para
		analizar en detalle los casos de pruebas presentados en este informe. Dada la extensión de dichos archivos, es
		inviable mostrarlos todos para cada caso de prueba, por lo cual, a modo de ejemplo, en el apéndice
		\ref{sec:logCasoPruebaArcoIris} se presenta un extracto del \emph{log} generado por Arco Iris para el caso que se
		detallará en la sección \ref{sec:testCase2}.
	...
	\subsection{Caso 4: \emph{Tradeoff} entre Escenarios según Prioridades}
		...
		En las figuras \ref{fig:testcase3_expRespTime} y \ref{fig:testcase3_cost} se puede observar el comportamiento del
		...
		\begin{figure}[H]
			\begin{center}
				\subfigure[Uso de prioridades favoreciendo al escenario de eficiencia por sobre el de costo]
						  {\label{fig:testcase3_expRespTime}\includegraphics[width=1.00\textwidth]{images/testcase3_expRespTime.png}}
				\subfigure[Reducción del costo como consecuencia de un cambio en el entorno]
						  {\label{fig:testcase3_cost}\includegraphics[width=1.00\textwidth]{images/testcase3_cost.png}}
			\end{center}
		...
		observa en la figura \ref{fig:testcase3_cost} de qué manera, a partir de los 5 segundos, el escenario relacionado con
		...
		En la figura \ref{fig:testcase3_cost}, aproximadamente a partir de los 33 segundos, se puede observar cómo Arco Iris
	...
	\subsection{Caso 5: \emph{Tradeoff} entre Escenarios según \emph{Concerns}}
		...
		acción llevar a cabo ante dicha situación. Puede apreciarse que Arco Iris opta por no reparar el escenario de tiempo de
		respuesta ya que esto perjudicaría al escenario de costo, puesto que si bien ambos poseen idéntica prioridad, el
		último está relacionado al \emph{concern} \textbf{costo de servidores}, el cuál en el entorno de ejecución actual
		(normal) tiene más peso que el \emph{concern} \textbf{tiempo de respuesta}.
	...
	\subsection{Caso 6: Comportamiento Ante Los Cambios en el Entorno de Ejecución}
		...
		de que el tiempo de respuesta no supere su umbral máximo para un entorno de alta carga (900 ms.). Este es el objetivo
		...
		escenario de costo posee más prioridad que el escenario de tiempo de respuesta para dicho entorno.
		...
		de respuesta que impide que se superen los 900 ms. Puede observarse en la figura \ref{fig:testcase6_expRespTime_cost}		que la auto reparación decide agregar un servidor aproximadamente a partir de los 23 segundos de iniciada la		simulación, reparando efectivamente el escenario en cuestión.
		\begin{figure}[H]
			\centering
				\includegraphics[width=1.0\textwidth]{images/testcase6_expRespTime_cost.png}
			\caption{Comportamiento del sistema en distintos entornos de ejecución}
			\label{fig:testcase6_expRespTime_cost}
		\end{figure}
