...
\label{sec:trabajoFuturo}
		...
		\subsubsection{Implementación difícil y definitiva}
			...
			\begin{itemize}
				\item \textbf{Actualización del mapa $<$Concern,Weight$>$ en todos los Entornos}
					...
					enumerados por extensión) y para eso se vale de la herramienta Scenarios UI. Básicamente, al agregar un nuevo
					\emph{Concern}, Scenarios UI automáticamente lo agregaría también en los mapas de todos los Entornos existentes en
					...
					serían todo este tipo de reajustes en la configuración para un usuario que no utiliza Scenarios UI\ldots
				...
				\item \textbf{Actualización del \emph{Concern} de Escenarios}
					...
					En el caso de dar de baja un \emph{Concern} lo más razonable sería que Scenarios UI lo elimine automáticamente de
				...
				\item \textbf{Asignación equidistribuída de pesos para un Entorno nuevo}
					...
					En Arco Iris, al construír una instancia de \verb@Environment@, se invoca a una función llamada
				...
				\item \textbf{Acceso del \emph{AdaptationManagerWithScenarios} a los \emph{Concerns}}

					Una situación similar a la anterior (el acceso al universo de \emph{Concerns} definidos en el sistema) ocurre
					también en el \verb@AdaptationManagerWithScenarios@:
					...
					El \verb@AdaptationManagerWithScenarios@ actualmente posee como colaborador interno a
					\verb@RainbowModelWithScenarios@ , el cuál accede al \verb@SelfHealingConfigurationManager@, quién es el que puede
					proveer el acceso deseado a todos los \emph{Concerns} definidos en el sistema en un momento dado.
			\end{itemize}
	...
	\subsection{Flexibilización del Entorno}
		...
		\verb@AdaptationManagerWithScenarios@ para que el mismo elija aquel Entorno con condiciones más generales. A
	...
	\subsection{Configuración de Escenarios en AcmeStudio}
		...
		incluso especular con un reemplazo completo de la herramienta Scenarios UI. Esta tarea no fue realizada en el
