...
\label{sec:trabajoFuturo}
	...
	\subsection{Configuración Dinámica y Ampliación de la Configuración Existente}

		En la sección \ref{sec:actualizacionDinamicaConfig} se explicó el trabajo hecho en Arco Iris en materia de recarga
		automática de la configuración. Recordemos que se implementó un mecanismo de recarga automática de la configuración
		relativa a Escenarios, Entornos, Artifacts y las referencias a las estrategias asociadas a cada escenario. Si bien tal
		...
		más útil en un ambiente dinámico como el de la industria del \emph{software}: es altamente deseable \textbf{recargar
		dinámicamente el 100\% de la configuración de Rainbow/Arco Iris}.
		
		Se propone como trabajo a futuro el ampliar el archivo de configuración de \emph{self healing} para que contemple la,
		hoy estática, configuración de Rainbow; así también como los nuevos conceptos presentados en Arco Iris pero que no son
		configurables externamente, como por ejemplo:

		\todo{mejorar lo de arriba y enumerar las cosas de Rainbow que pondríamos y las de Arco Iris, en dos enumerate
		distintos}
