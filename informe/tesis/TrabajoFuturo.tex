...
\label{sec:trabajoFuturo}
	...
	\subsection{Ampliación de la Recarga Dinámica de Configuración}
		...
		ambiente dinámico como el de la industria del \emph{software} dónde el tener que reiniciar la aplicación para aplicar
		un cambio en la configuración resulta muchas veces sencillamente inaceptable. Por lo tanto, es altamente deseable
		tender a \textbf{recargar dinámicamente el 100\% de la configuración de Rainbow/Arco Iris}. La recarga automática
		posee distintas características de acuerdo a si la misma ocurre en Rainbow o en Arco Iris. En los siguientes apartados
		daremos cuenta de ellas.

		\subsubsection{Implementación de la Recarga Dinámica de Configuración en Rainbow}

			\noindent \textbf{Qué se necesita hacer}\\
			\indent Como ya se ha dicho anteriormente en el presente documento, Rainbow no provee ningún tipo de refresco
			automático de configuración, es decir que para ver reflejados los cambios de configuración es necesario reiniciar el
			\emph{framework}.
			
			Rainbow actualmente maneja (a grandes rasgos) los siguientes puntos de configuración:
			\begin{itemize}
				\item el archivo \verb@rainbow.properties@, un típico archivo \verb@.properties@ leído por Java y que sirve para
				externalizar propiedades tales como el nivel de log deseado para la aplicación, el \emph{path} en dónde buscar
				estrategias y tácticas, el archivo .ACME con el modelo de la arquitectura del sistema a adaptar, etc.
				\item archivos con extensión \verb@.s@ (Stitch) de tácticas y estrategias.
				\item \verb@utilities.yml@, dónde se configuran las curvas de utilidad para el sistema.
			\end{itemize}
			
			\noindent \textbf{Cómo hacerlo / Posibles Problemas}\\
			\indent 
