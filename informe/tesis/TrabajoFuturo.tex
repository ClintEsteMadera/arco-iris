...
\label{sec:trabajoFuturo}
	...
	\subsection{Ampliación de la Recarga Dinámica de Configuración}
		...
		recarga ``en caliente'' de la configuración) entendemos que se puede dar un paso más en la materia, a fin de hacerla
		más útil en un ambiente dinámico como el de la industria del \emph{software} dónde el tener que reiniciar la
		aplicación para aplicar un cambio en la configuración resulta muchas veces sencillamente inaceptable. Por lo tanto, es
		altamente deseable tender a \textbf{recargar dinámicamente el 100\% de la configuración relacionada con auto
		reparación}. La recarga automática posee distintas características de acuerdo a si la misma ocurre en Rainbow o en
		Arco Iris. En los siguientes apartados daremos cuenta de ellas.

		\subsubsection{Implementación}

			Como ya se ha dicho anteriormente en el presente documento, Rainbow no provee ningún tipo de refresco automático de
			configuración, es decir que para ver reflejados los cambios de configuración es necesario reiniciar el \emph{framework}.
			
			Rainbow actualmente maneja (a grandes rasgos) los siguientes puntos de configuración:
			\begin{itemize}
				\item el archivo \verb@rainbow.properties@, un típico archivo \verb@.properties@ leído una única vez al
				inicializarse el \emph{framework} (en el constructor de la clase \verb@Rainbow)@ y que sirve para externalizar
				propiedades tales como el nivel de log deseado para la aplicación, el \emph{path} en dónde buscar estrategias y
				tácticas, el archivo .ACME con el modelo de la arquitectura del sistema a adaptar, etc. Sería relativamente
				sencillo implementar un mecanismo de recarga dinámica idéntico al explicado en detalle en la sección
				\ref{sec:actualizacionDinamicaConfig} para recargar la configuración de Arco Iris. 
				...
				``helper'' llamada \verb@YamlUtil@. Utilizando el mismo mecanismo descrito anteriormente se podría dinamizar también
				la recarga dinámica de este archivo.
				\item archivos con extensión \verb@.s@ (Stitch) de tácticas y estrategias. La lógica de lectura de tácticas
				y estrategias está ubicada en el \verb@AdaptationManager@. Puesto que en Arco Iris se necesitó extender esta clase,
				se aprovechó la oportunidad para abstraer esta lógica de lectura desde un archivo Stitch a una clase específica
				llamada \verb@StitchLoader@, la cual tiene la responsabilidad no solo de cargar los datos desde un archivo sino de
				proveer también acceso a los mismos. Sería una buena idea el incorporar esta clase al código base de Rainbow puesto
				que dicha abstracción provee la posibilidad de incorporar rápidamente el mecanismo de refresco mencionado ya
				reiteradas veces. Actualmente dicho comportamiento tampoco está incorporado en Arco Iris, no porque sea un
				trabajo difícil sino con la intención de cerrar el alcance del presente trabajo.
			\end{itemize}
		...
		\subsubsection{Ampliación de la Recarga Dinámica de Configuración en Arco Iris}

			%Qué se necesita hacer
			
			% Cómo hacerlo / Posibles Problemas

	\subsection{Ampliación de la Configuración Existente}

		Dividiremos los agregados/cambios a realizar de acuerdo a \emph{dónde} deben hacerse: en Rainbow, en Arco Iris o en
		Scenarios UI.
		...
		\subsubsection{Agregados a Rainbow}
			...
			\todo{Strategies y Tácticas, utilities.yml?}
