...
\label{sec:trabajoFuturo}
		...
		\subsubsection{Implementación difícil y definitiva}
			...
			\begin{itemize}
				...
				\item \label{asignacionEquidistribuidaPesos} \textbf{Asignación equidistribuída de pesos para un Entorno nuevo}
					...
					relacionados con la \emph{performance}, mientras que una aplicación de \emph{e-commerce} es probable que le de más
	...
	\subsection{Mecanismo de Reparación ``declarativo''}
		...
		Otra posibilidad menos programática y más declarativa podría consistir en que el usuario únicamente especifique, en el
		contexto del escenario, las tácticas que podrían ejecutarse en el caso en que el escenario no se cumpla; dejando al
		\emph{framework} la tarea de combinar ``inteligentemente'' dichas tácticas de la mejor manera posible. Considerando
		que la rama de las Ciencias de la Computación conocida como \textbf{Inteligencia Artificial} se encuentra en un
		estadío de poca maduración, la mentada ``inteligencia'' en realidad alude a que el \emph{framework} sea capaz de
		manejar heurísticamente los recursos que posee para interactuar con el sistema en \emph{runtime}, de una manera
		similar a cómo se opera actualmente con estrategias estáticamente configuradas por el usuario.
