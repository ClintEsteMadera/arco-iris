...
\label{sec:trabajoFuturo}
	...
	\subsection{Atributos de Calidad y \emph{Concerns} configurables por el usuario}
		...
		\subsubsection{Implementación difícil y definitiva}
			...
			\begin{itemize}
				...
				\item \textbf{Asignación equidistribuída de pesos para un Entorno nuevo}
	...
	\subsection{Flexibilización del Entorno}
		...
		de aplicabilidad parecen tener algún tipo de intersección no nula pues probablemente las condiciones necesarias para
		...
		seleccione a aquél que sea más general, en el ejemplo anterior, ``Extrema Alta Carga'' sería el elegido para
		representar más fielmente el estado actual del sistema. Esto sin duda posee una relevancia muy importante puesto que
		los pesos configurados en cada uno de los escenarios pueden determinar el curso de la auto reparación ya que afectan
		directamente al algoritmo de selección de estrategias.
	...
	\subsection{Optimización en la Selección de la Estrategia}
		...
		resolver una situación anómala en el sistema que se está adaptando. Claramente, este problema tiene una dimensión tal,
		que tratarlo en su justa medida requeriría un extenso trabajo de investigación, el cual excede al alcance del presente
		trabajo. A futuro, es necesario avanzar en este tema clave para el progreso y desarrollo de nuevas y mejores técnicas
		de auto reparación.
		
		La elección de la mejor estrategia posible para reparar un problema en el sistema ejecutandose, se puede complejizar
		tanto como uno desee, agregando heurísticas de distintos grados de complejidad y más inteligencia en general.
		
		La solución propuesta en el presente trabajo, el \emph{utility function}, si bien es útil para reducir la dimensión
		del problema y poder avanzar en otros aspectos, es claramente una solución limitada, debido a que para el cálculo del
		mismo, se realizan muchos supuestos y simplificaciones sobre el contexto de ejecución del sistema y su correlato con
		el modelo de su arquitectura; los cuales llevan a un grado de precisión definitivamente mejorable.

		\todo{TURCO: Fijate si podés aportar algo más a esta sección!}
	...
	\subsection{Alterar más radicalmente el comportamiento de un sistema}

		\todo{completar!}

		\ceria{
		Mencionen también las grandes líneas que se deben seguir explorando para hacer realidad esto de los sistemas
		``autoreparables''. Cómo armar una arquitectura de un sistema existente en ACME (lo de Discotect, pero que todavía es
		limitado), cómo ``enchufarse'' con un sistema sobre el que se tiene poco control, cómo poder alterar más radicalmente
		el comportamiento de un sistema, etc. Pueden poner estos temas en general, pero me gustaría que muestren que es muy
		importante seguir avanzando en sistemas que se ``manejan solos''.}
