...
\label{sec:trabajoFuturo}
	...
	\subsection{Ampliación de la Recarga Dinámica de Configuración}
		...
		\begin{itemize}
			...
			para recargar la configuración de Arco Iris. Dicho mecanismo tendría que ser realizado en el constructor de la clase
			\verb@Rainbow)@.
			...
			\verb@StitchLoader@, la cual tiene la doble responsabilidad de cargar los datos desde un archivo y de proveer también
			acceso dichos datos. Sería una buena idea el incorporar esta clase al código base de Rainbow puesto que
		...
		fácilmente reutilizable y también se observan posibilidades de mejora para simplificar aún más su uso extendido en
		...
		\subsubsection{Implementación difícil y definitiva}
			...
			\begin{itemize}
				\item \textbf{Actualización del mapa $<$Concern,Weight$>$ en todos los Entornos}
					...
					Una posible opción sería cambiar la semántica actual de dicho mapa, pasando a almacenar únicamente aquellos pesos
				...
				\item \label{asignacionEquidistribuidaPesos} \textbf{Asignación equidistribuida de pesos para un Entorno nuevo}
	...
	\label{sec:flexibilizacionEntorno}
		...
		que el entorno de ``Alta Carga'' se cumpla están incluidas en las condiciones el entorno de ``Extrema Alta Carga''.

		Es deseable que en el caso de entornos con intersección no nula en sus condiciones de aplicabilidad, Arco Iris
		seleccione a aquel que sea más adecuado, de acuerdo al caso en particular. En el ejemplo anterior, ``Extrema Alta
		...
		\verb@ArcoIrisAdaptationManager@ para que elija aquel Entorno con condiciones más generales. A continuación, un
		pseudo-código del posible algoritmo que contempla \textbf{inclusión entre Entornos}:
	...
	\subsection{Configuración de Escenarios en AcmeStudio}
		...
		características es una tarea compleja que no agrega valor al esfuerzo de investigación.
	...
	\subsection{Optimización en la Selección de la Estrategia}
		...
		resolver una situación anómala en el sistema que se está adaptando. Este problema posee muchas aristas complejas y por
		lo tanto, una solución sofisticada requiere un trabajo de investigación que excede lo que se puede realizar en este
		trabajo. Esta idea fue además compartida por los investigadores de Carnegie Mellon que fueron consultados al inicio
		del presente trabajo.
		
		La solución propuesta aquí al problema de selección de la mejor estrategia de reparación, es una heurística simple
		basada en el uso del concepto de \textbf{utilidad del sistema}, el cual, si bien es útil para reducir la dimensión del
	...
	\label{sec:ausenciaObsolescenciaModeloArquitectura}
		...
		grupo de investigación ABLE, el responsable de Rainbow y se llama \textbf{DiscoTect}. Para más información sobre dicho
	...
	\subsection{Adaptación de un Sistema Sobre el Cual se Tiene Poco Control}
		...
		los cuales parece poco probable que se puedan establecer puntos de comunicación con un \emph{framework} de auto
	...
	\subsection{Mecanismo de Reparación ``declarativo''}
		...
		basandose en observaciones sobre sus propiedades en \emph{runtime} y en un conjunto de tácticas (e.g. levantar un
		servidor extra, disminuir el nivel de \emph{logging}, apagar la encripción, etc\ldots) también provistas por el
