\section{Interfaz Gráfica: Scenarios UI}
	...
	\subsection{Conceptos básicos de uso de la herramienta}
		...
		\label{sec:scenariosUI_constraints}
			...
			arquitectura (e.g. \verb@server1.responseTime@) y un número (puede ser entero o flotante). Se permiten dos
			cuantificadores: sumatoria y promedio, los que especifican si la condición debe cumplirse para la suma de todas las
			instancias en \emph{runtime} de dicho componente o si la condición debe darse ``en promedio'' para todas aquellas
	...
	\subsection{Administración de Escenarios}
		...
		\subsubsection{Selección de Entornos}
			...
			\todo{DEBERIAMOS PONER ESTA EXPLICACION EN LA SECCION ``ARCO IRIS''??}
			\paragraph{El entorno ``ANY''}

				Existe un entorno no mostrado en la solapa de Entornos (ya que el mismo no es editable) pero que posee una
				relevancia no menor: el (pseudo-)entorno ``ANY'' (cualquiera) es una suerte de comodín utilizado para denotar la
				siguiente semántica: ``El presente escenario aplica para cualquier condición de \emph{runtime} (i.e. para cualquier
				entorno de ejecución). Consecuentemente, en dicho entorno especial, la distribución de pesos es la que uno puede
				esperar: los pesos se encuentran equidistribuídos para no ``inclinar la balanza'' hacia ningún \emph{concern} en
				particular y por otro lado, el entorno no impone ninguna condición, con lo cual el mismo aplicará siempre en todos
				los casos, trivialmente.
		...
		\subsubsection{Selección de Estrategias de Reparación}
			...
			el cual se encuentra definida la misma. Se elige primeramente el archivo que contiene las estrategias, para luego
	...
	\subsection{Puntos de extensión}
		...
		\begin{itemize}
			...
			cuestión.
			\item lo mismo se podría hacer con los estímulos, ya que los mismos serán normalmente la etiqueta de un puerto, en
			el contexto del diagrama de C\&C (componentes y conectores)
