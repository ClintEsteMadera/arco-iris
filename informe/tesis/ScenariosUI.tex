\section{Interfaz Gráfica: Scenarios UI}
	...
	\todo{TASK: (workaround temporal) Reemplazar los unicos 2 screenshots en Windows 7 por XP asi podemos presentar el
	informe más rapido}\\
	\todo{TASK: (solucion definitiva) Pasar todos los screenshots a Windows 7 así la app luce más moderna}
	...
	\subsection{Introducción}
		...
		\subsubsection{Herramienta de escritorio}
			...
			aplicaciones, al momento de definir cuestiones arquitecturales de una interfaz gráfica, pocos desarrolladores
			...
			Entonces bien, ¿por qué hemos elegido construir Scenarios UI basado en una interfaz gráfica de usuario de escritorio?
			...
			\begin{itemize}
				...
				\url{http://es.wikipedia.org/wiki/JFace}} resultan adecuados para modelar los diferentes controles visuales de la
				aplicación.
			\end{itemize}
	...
	\subsection{Conceptos básicos de uso de la herramienta}
		...
		\subsubsection{Formato de salida: XML}
			...
			La mecánica de serialización de una configuración de \emph{Self Healing} creada en Java utilizando Scenarios UI a
			...
			\emph{Self Healing} generado por Scenarios UI.
	
		\subsubsection{Una configuración de \emph{Self Healing} subyacente}

			La idea básica de uso de Scenarios UI es que, en todo momento, exista una configuración de \emph{Self Healing}
		...
		\subsubsection{Ids ``inteligentes''}
			El identificador único de los objetos que componen un escenario de auto reparación (e.g. \emph{artifacts}, entornos, etc.)
			es encarado en todos los casos de una única manera, tomemos el \emph{artifact} a modo de ejemplo: la aplicación
			considera los id's de los \emph{artifacts} existentes en el sistema y al momento de crear un \emph{artifact} nuevo,
			sugiere automáticamente un id no utilizado, de modo de garantizar la unicidad de dicho objeto cuando sea usado en ``Arco
			Iris''.
	...
	\subsection{Administración de Escenarios}
		...
		\label{sec:strategySelectionUI}
			...
			escenario, poder elegir una estrategia de reparación directamente desde el archivo \emph{stitch}\todo{referencia a
			la sección dónde mencionamos que las estrategias estan en stitch con detalle? (no en la seccion 2, sino en la 3)} en
			...
			\begin{figure}[ht]
				...
				\caption{Elección de todas las estrategias de reparación disponibles}
			\end{figure}
	...
	\subsection{Puntos de extensión}
		...
		Scenarios UI (en la sección \ref{sec:trabajoFuturo} se cubrirán los puntos de extensión para Arco Iris):
		\begin{itemize}
			\item \textbf{Obtener artefactos y estímulos desde el archivo .acme}: Es deseable el evitar mantener
			en Scenarios UI un ABM (\textbf{A}lta-\textbf{B}aja-\textbf{M}odificación) de artefactos, así también como que el
			...
			usuario es sin duda un editor de tácticas y estrategias integrado a Scenarios UI, con posibilidades de creación,
			edición y borrado. Asociado a ésto está la idea de que esta información sea guardada en el mismo archivo de
			configuración que el resto de la configuración producida por Scenarios UI. Esta idea será desarrollada con un
