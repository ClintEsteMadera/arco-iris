\section{Extensión a Rainbow: Arco Iris}
	...
	\subsection{ATAM para Flexibilizar la Auto Reparación}
		...
		\subsubsection{El Entorno en la Auto Reparación}
			...
			escenario automáticamente se cumple, ya que para considerar dicho escenario el sistema debería encontrarse en un			entorno de ``operación normal''.
			...
			adaptación pueda resultar muy costosa. Por el contrario, al configurar un valor elevado para \emph{alpha}, es			probable que el sistema se vea afectado por unos pocos valores que se encuentran por fuera del rango considerado como			normal, aunque de no ser costoso lanzar una adaptación o volverla atrás, podría llegarse a lograr un rendimiento			aceptable del sistema.
	...
	\subsection{Detección: cuando activar el mecanismo de auto reparación}
		...
		tipo de restricción permite fácilmente incorporar una nueva implementación implementando una \emph{interface} muy		sencilla (ver \ref{sec:numericBinaryRelationalConstraintCode}). Estas \emph{constraints} serán de vital importancia a		la hora de activar el mecanismo de auto reparación.		
