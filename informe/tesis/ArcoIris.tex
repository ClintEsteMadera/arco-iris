\section{Extensión a Rainbow: Arco Iris}
	...
	\subsection{Flexibilización de la Auto Reparación con QAS}
		...
		\subsubsection{El Estímulo en la Auto Reparación}
			...
			con otros \emph{probes}. De este \emph{bus}, los \emph{gauges} consumirán la información para luego interpretarla y
	...
	\label{sec:strategies}
		...
		\ref{sec:strategySelectionUI}). Implementativamente, basta sólo con asignarle al escenario la única instancia de la
		clase \verb@AllRepairStrategies@.
	...
	\subsection{Activación del Mecanismo de Auto Reparación}
		...
		\subsubsection{Introducción}
			...
			determinado umbral es necesario definir una restricción en el componente que posee dicha propiedad (i.e. el tiempo
			de respuesta), como se puede observar en el siguiente ejemplo tomado de la arquitectura de Znn:
		...
		\label{sec:desarrolloMecanismoAutoReparacion}
			...
			\begin{Verbatim}[gobble=4]
				public class RainbowModel {
					...
					public void evaluateConstraints () {
						...
						m_constraintViolated = !typechecker.typechecks(m_acmeSys);
			...
			mecanismo de parseo de restricciones y en cambio, maneja su propio modelo simplificado de restricciones, el cual se
			encuentra desacoplado del modelado de la arquitectura. Estas restricciones (o \emph{constraints}) serán de vital
			importancia al momento de activar el mecanismo de auto reparación.
			
			Al iniciar, Arco Iris lee todos los escenarios definidos (más detalle, en la sección
			...
			relacionados con aquel estímulo, acotando así de manera sustancial la cantidad de chequeos a realizar. Recordar			que en Rainbow, ante cualquier modificación en el estado de la arquitectura, es necesario verificar todas las			restricciones definidas para la auto reparación.
	
			\ceria{poner un grafico que ilustre este proceso mejor!}
			...
			\begin{enumerate}
				...
				información sobre dicho evento en un \emph{bus} compartido (ya existente en Rainbow).
				\item Un \emph{gauge} configurado para ``escuchar'' este tipo de eventos, toma el mensaje y lo traduce a un
				mensaje que incluye una tripla \verb@<Propiedad,Valor,Estímulo>@, la cual a su vez será depositada en una cola.
				\item El componente \emph{gauge coordinator} chequea periódicamente dicha cola en búsqueda de nuevas instrucciones
				para actualizar el modelo arquitectural con los cambios ocurridos en \emph{runtime}. Al encontrar el mensaje dejado por
				el \emph{gauge}, informará a \verb@ArcoIrisModel@ (una extensión del \verb@RainbowModel@) sobre dicho cambio de
				propiedad y el estímulo que lo generó.
				\item
					\begin{enumerate}
				...
				automáticamente.
				\item En caso de detectar que uno o más escenarios dejaron de cumplirse, el \verb@ArcoIrisModel@ instruye a
				\verb@ArcoIrisAdaptationManager@ (una extensión del \verb@AdaptationManager@ de Rainbow) para que inicie la auto
				reparación del sistema.
				\item En primer término, el \verb@ArcoIrisAdaptationManager@ debe decidir cual será la estrategia más conveniente a
	...
	\label{sec:strategySelection}
		...
		\todo{Agregar diagramas conceptuales en las partes que de la sección dónde necesite para clarificar un poco las
		...
		considerar únicamente las estrategias propuestas en el escenario que se ha dejado de satisfacer, esto obviamente
		reduce ampliamente el universo de posibilidades y consecuentemente, el tiempo de la auto reparación.
		
		Como se menciona en la sección \ref{sec:strategies}, Arco Iris también ofrece la posibilidad de considerar todas las
		estrategias existentes, sin enumerarlas explícitamente, logrando así que tal configuración sea escalable ante el
		agregado de nuevas estrategias al sistema. Si bien esta decisión evita al usuario el tener que realizar una
		configuración más rigurosa del \emph{framework}, al mismo tiempo va en desmedro de la eficiencia de la auto
		reparación, por los motivos ya explicados en la sección \ref{sec:strategies}.
		...
		\label{sec:systemUtility}
			...
			\ceria{No habría que explicarlo más conceptual? Algo así como ``un sistema brinda un 100\% de utilidad cuando cumple
			todos sus requerimientos y no viola ninguna de las restricciones planteadas en sus escenarios''. En la medida en que
			los escenarios no se cumplen, la utilidad del sistema baja. Si un conjunto de escenarios pueden estar rotos y el
			Framework puede decidir cuáles la combinación, es lógico qeu busque maximizar la utilida. O ALGO ASI.}
			...
			impacto ante determinados cambios (i.e. aplicación de una estrategia). Una vez definida esta medida, es posible
			determinar si la potencial aplicación de una estrategia de reparación aumentaría o disminuiría la utilidad del
			sistema. En otras palabras, Rainbow posee un mecanismo para juzgar qué estrategia dejaría al sistema en una mejor
			situación, de acuerdo al valor arrojado por el cálculo de la utilidad del sistema luego de la supuesta ejecución de
			cada una de las estrategias candidatas.
			...
			\begin{itemize}
				...
				\item por cada uno de los \emph{concerns} que manejará el sistema, se define la función de utilidad, la cual, dada
			...
			\begin{Verbatim}[gobble=4]
				...
				utilities:
					...
					uF:
						...
						description:  "Server content fidelity in level, F, defined as an int property
						              'ServerT.fidelity' in the architecture, with some artificial range
						              to be determined"
						utility:
							0:    0.00
							5:    1.00
			...
			Se observa que \verb@uR@ configura el tiempo de respuesta promedio experimentado por el usuario, dicho \emph{concern}
			se encuentra emparentado con la propiedad \verb@experRespTime@ del tipo de componente \verb@ClientT@.
			\todo{mencionar lo de EAvg???} También se observa que, por ejemplo, para un valor de 2000 milisegundos en el tiempo
			de respuesta experimentado por un usuario, la función de utilidad arroja un valor de 0.25. En breve, se analizará el
			uso que Rainbow hace de este valor.

			A continuación, se muestra el pseudocódigo del cálculo de la denominada \emph{Utilidad del Sistema} en Rainbow:
			...
			En el caso de Rainbow, la utilidad del sistema permitirá tener una noción de como impactará una determinada
			estrategia sobre el sistema. Considerando que las estrategias se componen de tácticas, cada táctica deberá explicitar
			los \emph{concerns} sobre los cuales impacta y en qué medida lo hace. Por ejemplo, en el caso de ``bajar un servidor'' en
			Znn se especifica la siguiente meta información asociada a la táctica, representando el impacto estimado sobre el
			sistema:
			\begin{Verbatim}[gobble=4]
				dischargeServers:
					uR: +144
			...
			Esto significa que una vez ejecutada esta táctica, de manera aproximada, la eficiencia se verá degradada en
			144 milisegundos y la fidelidad permanecerá intacta, mientras que el costo se reduce en 1.
		...
		\label{sec:rainbowStrategyScoring}
			...
			Una vez obtenido el subconjunto de estrategias aplicables, Rainbow procede a asignarle un puntaje a cada una, un
			valor entre 0 y 100, y luego \textbf{aplica la estrategia de mayor puntaje}.
			...
			\begin{enumerate}
				...
				la estrategia en cuestión y aplica a dichos valores las respectivas funciones de utilidad definidas en el archivo de
				configuración \verb@utilities.yml@. (ver sección \ref{sec:systemUtility})
			\end{enumerate}

			A fin de comprender la heurística aplicada por Rainbow para obtener las estimaciones explicadas en el punto
			\ref{estimaciones}, es útil recordar que las mismas poseen líneas etiquetadas (e.g. \verb@t0, t1@, etc...) las cuales
			pueden entenderse como ``ramas'' del algoritmo, por ejemplo:
			...
			presentes en cada rama, esto es, el impacto que cada táctica tiene sobre los \emph{concerns} del sistema,
			concepto explicado previamente en la sección \ref{sec:systemUtility}. Estos valores luego serán ponderados por el
			peso estático de cada \emph{concern} (punto \ref{pesosEstaticos}) para obtener finalmente el puntaje de la
			estrategia.
		...
		\label{sec:arcoIrisStrategyScoring}
			...
			la estrategia, escogiendo, finalmente, para reparar el sistema a aquella estrategia que posea el puntaje máximo.
	...
	\label{sec:configuracionArcoIris}
		...
		\todo{Hacer un diagrama de la clase SelfHealingConfiguration con UML}
