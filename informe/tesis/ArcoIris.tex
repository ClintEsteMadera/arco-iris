\section{Extensión a Rainbow: Arco Iris}
	...
	\subsection{Rainbow \emph{out of the box}}
		...
		\todo{Agregar en ambos graficos: AdaptagionManager escoge y lanza Strategy (opcional)}
		\todo{Agregar en ambos graficos: Effector invocado desde el Strategy o el Adaptation Manager}
		...
		\begin{enumerate}
			\item la creación de un modelo de la arquitectura del sistema al cual Rainbow se va a adaptar. Dicho modelo se
		...
		Podemos ver también en el gráfico anterior que los denominados \emph{Probes} son los componentes designados en
		...
		delegan dicha tarea a los denominados \emph{Gauges}. Estos componentes son los encargados de interpretar la
		información provista por los \emph{probes} y traducirla a pares $<Propiedad,Valor>$ donde $Propiedad$ refiere al
		nombre de una propiedad correspondiente a un componente (o conector o sub sistema, etc\ldots) de la arquitectura del
		sistema a adaptar; y $Valor$ es el nuevo valor que posee dicha propiedad.

		Normalmente, por cada tipo relevante de \emph{concern} que interese ser monitoreado en tiempo de ejecución existe un
		par $<Gauge,Probe>$ asociado (que a nivel implementativo son meras clases Java). Un ejemplo de \emph{concern} podría
		...
		sub sistemas o sistemas del modelo de arquitectura de la aplicación a adaptar. También es quien tiene el
		conocimiento necesario para detectar violaciones a las restricciones e invariantes presentes en el modelo de
		arquitectura con el cual el \emph{framework} ha sido configurado.
		...
		(explicada en detalle en la sección \ref{sec:strategySelection}) para determinar la mejor estrategia de reparación a
		ejecutar, la cual luego será ejecutada mediante \emph{Effectors} (no mostrados en el gráfico anterior por claridad)
		siempre intentando acercar el estado del sistema a sus invariantes preestablecidos.
	...
	\subsection{Rainbow + Escenarios = ``Arco Iris''}
		...
		\subsubsection{Arquitectura de Arco Iris}
			...
			modelo de la arquitectura del sistema a adaptar y con archivos de configuración requeridos por Rainbow) pero ahora se le suma una tarea más:
			...
			del sistema, líderes, clientes, el \emph{sponsor} del proyecto, etc.)
			...
			En el anterior diagrama se puede observar que tanto los \emph{probes} como los \emph{gauges} han de  tomar
			conocimiento del concepto de ``estímulo'' (tal cual es descrito en los escenarios de atributos de calidad de ATAM).
			A diferencia de los empleados en Rainbow, los  \emph{probes} implementados para trabajar con Arco Iris deberán
			indicar el estímulo al cual representan, mientras que los \emph{gauges} deberán interpretar esta información y
			notificar la actualización sobre el modelo indicando el estímulo desencadenador. Las novedades sobre los cambios
			sucedidos en el sistema en \emph{runtime} serán informados con el siguiente formato: $<Propiedad,Valor,Estimulo>$

			Puesto que en la presente extensión el componente \emph{Gauge} provee información sobre los estímulos que desencadenaron
			un cambio de propiedades en el sistema en ejecución, el componente \emph{Gauge Coordinator} ha sido extendido (i.e.
			\emph{Gauge Coordinator With Scenarios}) para soportar dicho agregado de información, sin perder el comportamiento
			original provisto por Rainbow. Esto hace que \emph{Gauge Coordinator With Scenarios} pueda coordinar información
			proveniente de \emph{gauges} de Rainbow y/o \emph{gauges} de Arco Iris con su correspondiente conocimiento sobre el
			estímulo originario de la modificación.
			...
			provisto por \emph{Rainbow Model}.
			...
			dejado de cumplirse\footnote{De ahora en más, diremos equivalentemente que el escenario en esta situación se encuentra
			``roto''.}. Luego de obtener este subconjunto de escenarios, procederá a invocar al \emph{Adaptation Manager With
			Scenarios}, extensión del componente original \emph{Adaptation Manager} que soporta tanto invocaciones desde el
			\emph{Architecture Evaluator} original de Rainbow, como así también invocaciones con un conjunto de escenarios
			...
			También será responsabilidad de \emph{Rainbow Model With Scenarios} cargar los escenarios de atributos de calidad   
			creados por los \emph{stakeholders} y arquitecto(s), así como también detectar los escenarios rotos, para lo cual   
			contará con un componente colaborador llamado \emph{Self Healing Configuration Manager} , el cual cumple la función   
			de abstraer toda la información referente a auto reparación agregada por Arco Iris.			
			\todo{Agregar Self Healing Configuration Manager en el grafico como un colaborador de Rainbow Model With Scenarios}
	\subsection{ATAM para Flexibilizar la Auto Reparación}
		...
		\subsubsection{El Estímulo en la Auto Reparación}
			...
			iteración, tal cual lo hace Rainbow hoy en día.
			...
			que son los componentes encargados de \emph{testear} una determinada funcionalidad del sistema, siendo el responsable
			también de medir la respuesta y de reportar dicha información a un bus, del cual luego la consumirán los \emph{gauges},
			quienes interpretarán los datos transformándolos en cambios en el modelo de la arquitectura. 
			
			Como ya se mencionó anteriormente, fue necesario extender la funcionalidad de los \emph{gauges} para poder intrepretar
			la información de estímulo agregada en cada uno de los \emph{probes}. En ambos casos, y gracias al mecanismo empleado
			por Rainbow, no fue necesario modificar las interfaces de los \emph{probes} ni de los \emph{gauges}, ya que éstos se
			comunican mediante cadenas de texto, justamente para ofrecer una amplia libertad a la hora de implementarlos ya que
			pueden existir diversas maneras de probar y tomar métricas de las distintas funcionalidades de los sistemas. Luego, una
			vez obtenido el estímulo correspondiente, no resta más que recurrir al mapa de escenarios por estímulos (utilizado
			también en el modo simulación) para acotar la cantidad de escenarios a comprobar.
			\todo{agregar comparación de info reportada por los Probes con estimulos y sin estimulos (no lo encontré!!)}
		...
		\subsubsection{El Artefacto en la Auto Reparación}
			...
			La sección \ref{sec:scenariosUI} explica en detalle cómo se podrá emplear esta información para mejorar la
			experiencia de los \emph{stakeholders} a la hora de especificar escenarios mediante la herramienta visual presentada
			junto a Arco Iris: \textbf{scenarios UI}
		...
		\subsubsection{El Entorno en la Auto Reparación}
			...
			en que se recibe el estímulo. 

			Es importante mencionar que si bien según la definición de ATAM, cada escenario posee un único entorno, en Arco Iris se
			ha tomado la decisión de diseño, por razones de flexibilidad, de permitir configurar varios entornos por escenario,
			siendo obligatorio indicar al menos uno.\todo{Explicar un poco mas esta decision ya que no es trivial}
			Arco Iris también provee la posibilidad de especificar si un determinado escenario aplica bajo cualquier entorno, esto
			sería equivalente a que el entorno del escenario tenga como única condición subyacente \verb@true@. Esto puede ser útil
			para simplificar la configuración del escenario, pero es importante tener en cuenta que esto acota la posibilidad de
			obtener un rendimiento más óptimo de la auto reparación para con el sistema, ya que configurar un entorno específico
			permite mayor precisión al configurar las prioridades relativas de los \emph{concerns} según el escenario    y el estado
			del sistema.
