\section{Extensión a Rainbow: Arco Iris}
	...
	\subsection{ATAM para Flexibilizar la Auto Reparación}
		...
		\subsubsection{El Entorno en la Auto Reparación}
			...
			\todo{TURCO: Esto era un To-Do para que yo opine: ``Explicar que Constraint define el metodo boolean holds(Number
			value) y que la unica implementacion es NumericBinaryRelationalConstraint.???'' Fijate que vos mismo ya explicás eso
			en la sección ``La Cuantificación de la Respuesta en la Auto Reparación'' y que yo algo expliqué de eso en la
			sección Scenarios UI, fijate de poner la misma info en un sólo lado\ldots quizás convenga ponerlo acá y sacarlo de
			la sección de Scenarios UI\ldots}
		...
		\subsubsection{La Cuantificación de la Respuesta en la Auto Reparación}
			...
			\todo{TURCO: poner la implementación de NumericBinaryRelationalConstraint en el apéndice}

	\subsection{Prioridades entre Escenarios}

		\todo{TURCO: Esto está confuso, aclarar que nosotros usamos escenarios de QAW pero que los llamamos por simplicidad
		``escenarios'', dejarle al lector más claro este punto.}

		Rainbow maneja el concepto de escenario, aunque no es equivalente al manejado por Arco Iris, el cual toma el concepto
		de Escenario de QAW de la definición de ATAM. Desde la perspectiva de Rainbow, el escenario es algo configurable pero
		estático, es decir, no ofrece la posibilidad de adaptarse a los cambios del entorno, por lo que el arquitecto deberá
		configurar el escenario\footnote{Recordar que aquí escenario no es el mismo concepto utilizado por Arco Iris, tan sólo
		se reutiliza con la finalidad de explicar el funcionamiento de Rainbow.} previo al inicio de la herramienta,
	...
	\subsection{Detección: cuando activar el mecanismo de auto reparación}
		...
		evaluar sus restricciones o invariantes).
