\section{Extensión a Rainbow: Arco Iris}
	...
	\subsection{Flexibilización de la Auto Reparación con QAS}
		...
		\subsubsection{El Estímulo en la Auto Reparación}
			...
			\todo{TURCO: implementar el agregar estimulo en los simulation points}
		...
		\label{sec:environment}
			...
			\subsubsubsection{El Entorno y su Importancia para la Auto Reparación}

				Dentro de la definición de escenario, el \textbf{entorno} se refiere al estado en el que el sistema se encuentra
				cuando recibe el estímulo que desencadena el escenario. A modo de ejemplo: al recibir una solicitud de creación de
				una cuenta bancaria el sistema puede encontrarse en ``operatoria normal'', en ``alta carga'' o quizás tal vez se
				encuentre ``fuera de operación''.
				
				Como se observa, el entorno condiciona la validez del escenario en cuestión a que el sistema se encuentre en un
				determinado estado, ya que, por ejemplo, una respuesta esperada aceptable o fácil de cumplir bajo un entorno puede ser
				inaceptable o muy costosa en otro.
	
				En el escenario planteado en la sección anterior, si el sistema se encuentra en un entorno de ``alta carga'' el
				escenario se satisface trivialmente, ya que para considerar dicho escenario el sistema debería encontrarse en un
				entorno de ``operación normal''.
			...
			\subsubsubsection{Escenarios modelados con varios entornos}
				...
				únicamente un sólo escenario, en Arco Iris se ha decidido modelar al escenario con una \textbf{colección de
				...
				sistema dos o más entornos cuyas condiciones de aplicabilidad tengan intersección no nula. Esta simplificación, si
				bien no configura mayor problema a los fines de este trabajo, si restringe futuras extensiones del mismo. Tal
				restricción es abordada en detalle en la sección \ref{sec:flexibilizacionEntorno}.
	...
					La implementación de las \textbf{condiciones} fue inspirada  en el concepto de \textbf{restricción} ya presente en
				Rainbow, el cual permite definir precondiciones de aplicabilidad para las tácticas y las estrategias.
				
				En Arco Iris se utilizará el mismo concepto de condición en dos contextos distintos, ninguno de los cuales se
				corresponde exactamente con la concepción original de Rainbow. Por un lado, se utilizarán las condiciones				para conocer el entorno en que se encuentra el sistema, y por otro, la cuantificación de la respuesta (ver sección				\ref{sec:responseMeasure}) estará conformada por varias condiciones que determinarán aquellas restricciones				necesarias para que el escenario se satisfaga. En la mentada sección se explica la diferencia conceptual del				concepto de restricción utilizado en Rainbow con el de Arco Iris. \todo{verificar si se explica esta diferencia}	
				\todo{Jony: continuar revisando desde acá}
	
				El concepto de condición (o restricción) presentado en Arco Iris permite agregar nuevos tipos de
				restricciones simplemente implementando la interfaz \verb@Constraint@ \footnote{Dicha implementación deberá ser
				agregada al enumerado ConstraintType en Arco Iris UI para que la UI reconozca el nuevo tipo}. 
				
				En cuanto al \textbf{mapa de pesos} presente en la estructura del entorno, este deberá contener todos los
				\emph{concerns} definidos para el sistema, y obviamente la suma de los pesos debe ser igual a uno, ya que la
				utilidad del mapa es determinar la preponderencia relativa entre todos los \emph{concerns} de interés para el
				sistema. Se detallará cómo se utilizarán estos pesos para calcular el valor de un escenario al escoger una
				estrategia en la sección \ref{sec:arcoIrisStrategyScoring}.
			...
			\subsubsubsection{El Entorno ``ANY''}
				...
				Arco Iris también provee la posibilidad de especificar, mediante la selección de un entorno \emph{virtual}
				preexistente denominado ``ANY'', que un determinado escenario aplica bajo cualquier entorno. Esto es equivalente a
				que el entorno del escenario no posea condición alguna, es decir, que el escenario aplica siempre, trivialmente, sin
				importar las condiciones actuales del sistema en ejecución. Esto puede resultar útil en algunos casos ya que
				simplifica la configuración del escenario, pero es importante tener en cuenta que, por otro lado, reducirá el
				rendimiento de la auto reparación ya que el configurar un entorno específico por escenario otorga mayor precisión al
				permitir establecer las prioridades relativas de los \emph{concerns} según el estado en el que el sistema se
				encuentre.
	
				El entorno ``ANY'' será asignado automáticamente al escenario si el usuario no escoge ningún entorno o, si por el
				contrario, decide explícitamente que el escenario aplique \textbf{en cualquier entorno de ejecución}. En
				cualquiera de los dos casos, todos los \emph{concerns} poseerán el mismo peso, evitando así otorgarle más
				importancia a algún \emph{concern} en particular. Esta decisión de equidistribuír los pesos relativos por
				\emph{concern}
		...
		\subsubsection{La Cuantificación de la Respuesta en la Auto Reparación}
			...
			consiste en el momento en que se aplican. Hemos visto anteriormente que Rainbow posee un componente llamado \emph{``Architecture Evaluator''}, cuya
	...
	\label{sec:modeloExtensionATAM}
		...
		En la figura \ref{fig:Arco_Iris_QAS_Model} se puede observar cómo se modela el \textbf{escenario de auto reparación}
	...
	\label{sec:strategies}
		...
		\todo{Este ejemplo lo trajimos de la seccion ``El estimulo en la auto reparacion. Revisar!}
		...
		en Arco Iris existe la posibilidad de evitarlo al configurar correctamente las estrategias asociadas a cada escenario.
