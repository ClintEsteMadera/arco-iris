\section{Extensión a Rainbow: Arco Iris}
	...
	\label{sec:strategySelection}
		...
		configuración de Arco Iris también ofrece la posibilidad de considerar todas las estrategias existentes.

		Por su parte, Rainbow descartará las estrategias cuyas condiciones no se satisfagan, están condiciones generalmente se
		...
		En Arco Iris, una vez que se ha detectado que un escenario dejo de satisfacerse, simplemente se toman como estrategias
		candidatas las definidas en él, evitando así el \emph{overhead} de volver a verificar las condiciones.
		...
		\label{sec:systemUtility}
			...
			\ref{rainbowScenario}). La suma ponderada de los valores de todos los atributos de calidad dará como resultado un
			único valor, al que llamaremos \emph{Utilidad del Sistema}.
			En el caso de Rainbow esta medida permitirá tener una noción de como impactará una determinada estrategia sobre el
			sistema, para lo cual cada táctica deberá explicitar los \emph{concerns} sobre los cuales impacta y en qué medida
			lo hace. Por ejemplo, en el caso de bajar un servidor en Znn se especifica la siguiente metainformación asociada la
			...
			milisegundos y la fidelidad permanecerá intacta, mientras que el costo se reduce en 1, respectivamente.

			\todo{La utilidad del sistema se calcula diferente en ambos frameworks, explicar!}

		\subsubsection{Puntuación de estrategias según Rainbow}

			Una vez obtenido el subconjunto de estrategias aplicables, Rainbow precederá a asignarle un puntaje a cada
			...
			\begin{Verbatim}[gobble=4]
				...
				Por cada concern
					...
					valorPonderado = valorUtilidad * peso asignado al concern en el escenario
		...
		\label{sec:arcoIrisStrategyScoring}
			...
			siguiente:
			\begin{itemize}
				\item el entorno de ejecución en que se encuentra la aplicación y los pesos que el mismo asigna a cada
				\emph{concern},
			...
			Para calcular la utilidad del sistema, Arco Iris asignará un determinado puntaje a cada escenario habilitado que			se satisfaga, el resto de los escenarios serán ignorados, o lo que es equivalente, tendrán puntaje cero. Por cada			escenario habilitado Arco Iris verificará si el mismo aplica para el entorno actual del sistema, en caso de aplicar
			el escenario se satisface trivialmente, por lo sumará a la utilidad del sistema. Para los escenarios que si apliquen
			al entorno actual, Arco Iris evaluará si las condiciones del mismo se satisfacen y sólo en caso afirmativo sumarán a
			la utilidad del sistema.
			
		\todo{EXPLICAR MAS DE LA SIMULACION?}
			...
					\todo{JONY: verificar refactoring para poder continuar escribiendo esta seccion!!}
		...
		El objetivo final de Arco Iris es el de intentar reparar el inconveniente hallado en el sistema pero siempre
		considerando que se debe evitar perjudicar algún otro escenario de mayor prioridad \todo{Esto es un efecto colateral
		de la utility function pero...podemos garantizar que esto es asi siempre? Creo que esto necesita ser reescrito} y
		mediante el uso de heurísticas, poder aproximar la mejor estrategia de reparación a llevar a cabo de modo que la
		\emph{utilidad del sistema} se maximize.
