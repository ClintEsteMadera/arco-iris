\section{Extensión a Rainbow: Arco Iris}
	...
	\subsection{ATAM para Flexibilizar la Auto Reparación}
		...
		\subsubsection{El Entorno en la Auto Reparación}
			...
			entorno carece de importancia, dejando así de lado los pesos de los \emph{concerns} y distribuyendo equitativamente			su importancia relativa, lo cual es equivalente a que no exista el concepto de \emph{concern}.
			...
			sistema, y obviamente la suma de los pesos debe ser igual a uno, ya que la utilidad del mapa es determinar la			preponderencia relativa entre los \emph{concerns}. Se detallará cómo se utilizan estos pesos para calcular el valor			de un escenaio en la sección \ref{sec:scenarioWeight}.
			...
			Un valor recomendable para el factor de suavizado (\emph{alpha}) utilizado tanto en las pruebas de Rainbow como en
			las de Arco Iris es 0.3. Si se configura un valor muy pequeño, Arco Iris tardará en adaptarse a los cambios en el
			entorno del sistema. Ésto podría ser útil, por ejemplo, en sistemas con bajo grado de dinamismo o en los cuales la
			adaptación pueda resultar muy costosa. Por el contrario, al configurar un valor elevado para \emph{alpha}, es			probable que el sistema se vea afectado por unos pocos valores que se encuentran por fuera del rango considerado como			normal, aunque de no ser costoso lanzar una adaptación o volverla atrás, podría llegarse a lograr un rendimiento			aceptable del sistema.
	...
	\label{sec:strategySelection}
		...
		\label{sec:systemUtility}
			...
			lo hace. Por ejemplo, en el caso de bajar un servidor en Znn se establece el siguiente impacto:
			...
			Esto representa que la performance se verá degrada en 144 milisegundos y la fidelidad permanecerá intacta, mientras
			que el costo se reduce en 1, respectivamente.

		\subsubsection{Valoración de estrategias según Rainbow}

			Una vez obtenido el subconjunto de estrategias aplicables, Rainbow se encargará de asignarle un puntaje a cada
			una, el cual será un valor entre $0$ y $100$, y luego aplicará la estrategia de mayor puntaje. Para calcular el
			puntaje, lo primero que hará Rainbow será obtener los pesos asignados a cada \emph{concern}(e.g. costo, fidelidad,
			etc) según el escenario seleccionado (recordemos que el escenario ha sido configurado previo inicio de Rainbow), con
			los cuales pesará los valores estimados para cada \emph{concern} luego de la potencial aplicación de la	estrategia.

			La heurística aplicada por Rainbow para estimar el valor de cada \emph{concern} una vez aplicada la estrategia
			resulta de considerar la probabilidad de que se ejecute cada rama de la estrategia y los atributos de las tácticas
			presentes en cada rama. Estos valores luego serán ponderados por el peso de cada \emph{concern} para obtener			finalmente el puntaje de la estrategia. En la figura \ref{fig:scoreStrategyCode} se muestra un pseudocódigo del			cálculo de puntuación de una estrategia dada.

			\begin{Verbatim}[gobble=4]
				puntaje = 0
				Estimar los valores para cada concern luego de aplicar la estrategia
				Por cada concern
					uf = Funcion de Utilidad para el concern actual
					estimacion = valor estimado para el concern
					...
					\label{fig:scoreStrategyCode}
			\end{Verbatim}

		\subsubsection{Valoración de estrategias según Arco Iris}

			\todo{mencionar que primero se calcula la utility actual para no empeorar las cosas}

	 El mismo ha sido extendido \todo{¿cómo?} para que utilice el conocimiento plasmado en los
