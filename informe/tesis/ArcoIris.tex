\section{Extensión a Rainbow: Arco Iris}
	...
	\subsection{ATAM para Flexibilizar la Auto Reparación}
		...
		\subsubsection{La Cuantificación de la Respuesta en la Auto Reparación}
			...
			con suficiente tiempo y dinero, cualquier modificación es posible. Este escenario requiere una métrica, como por
			...
			El siguiente ejemplo ha sido extraído de la arquitectura de Znn (expresada utilizando Acme), allí se puede observar
			cómo se implementan las restricciones en Rainbow, las que pasan a formar parte de la cuantificación de la respuesta del
			escenario al utilizar Arco iris:
			...
			Es importante recordar que en Rainbow, una vez que se detectó que la auto reparación debe ser lanzada, se volverán
			a ejecutar todas las restricciones dictadas en las precondiciones de las estrategias para saber si aplican en el
			estado actual del sistema, el siguiente ha sido extraído de Znn:
			...
			estrategias son capaces de reparar una determinada condición ya se encuentra plasmado en cada escenario. En
			definitiva, al utilizar Arco Iris el usuario no tendrá más que configurar la cuantificación de la respuesta de cada
			escenario, tarea que se ve sumamente facilitada al utilizar Scenarios UI (ver sección~\ref{sec:scenariosUI} en la
			página~\pageref{sec:scenariosUI}).

			Otra diferencia vital ante el uso de las mencionadas restricciones entre la visión de Rainbow y de Arco Iris consiste en
			el momento en que se aplican. Rainbow posee un componente llamado \emph{``Architecture Evaluator''}, cuya
			...
			absolutamente todas las restricciones. Mientras que Arco Iris, al descubrir un cambio en el modelo de la arquitectura,
			la acción que tomará consistirá en verificar los escenarios, pero no todos los escenarios, sino solamente los que
			coincidan en su estímulo con el estímulo que desencadenó el cambio en el modelo, optimizando así de manera determinante
			la cantidad de restricciones a verificar, lo que hace mucho más escalable la auto reparación.
			...
			considere necesario. La interfaz presentada es la siguiente:
