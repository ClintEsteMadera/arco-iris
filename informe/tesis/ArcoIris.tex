\section{Extensión a Rainbow: Arco Iris}
	...
	\subsection{Flexibilización de la Auto Reparación con QAS}
		...
		\subsubsection{El Artefacto en la Auto Reparación}
			...
			El \textbf{artefacto} se refiere al componente, conector o sub sistema afectado por el escenario, los cuales se
			encuentran descritos en el modelo de la arquitectura.

			La asociación de un artefacto en particular a un escenario permite acotar las propiedades sobre las cuales podrá
			predicar la \textbf{cuantificación de la respuesta} (ver \ref{sec:responseMeasure}).

			La arquitectura presentada en Znn servirá para ejemplificar el concepto de artefacto, cabe recordar que se trata de
			una arquitectura cliente-servidor, donde el componente de tipo \emph{ClientT} se define de la siguiente manera:  
		...
		\label{sec:environment}
			...
			cuenta bancaria el sistema puede encontrarse en ``operatoria normal'' o en ``alta carga''. 
			
			El entorno condiciona la validez del escenario en cuestión a que el sistema se encuentre en un determinado estado, ya
			que, por ejemplo, una respuesta esperada aceptable o fácil de cumplir bajo un entorno puede ser inaceptable o muy
			costosa de cumplir en otro entorno.
			En el escenario planteado anteriormente en la sección \ref{escenarioPerformance}, si el sistema se encuentra en un
			entorno de ``alta carga'' el escenario automáticamente se satisface, ya que para considerar dicho escenario el
			sistema debería encontrarse en un entorno de ``operación normal''. 
			...
			\todo{explicar y justificar porque un QAS tiene varios escenarios}\\
			\todo{Ejemplo de mail: con un solo entorno por escenario, si quisieramos que un escenario se cumpla en varios			entornos, deberiamos crear un escenario por cada entorno, cada uno de los cuales solo difiere en el entorno}\\			\todo{Interseccion de los entornos? El sistema en un momento puede estar en mas de un entorno?}
			\todo{Ver consistencia con ``Flexibilizacion del entorno'' en Trabajo Futuro}
