\section{Extensión a Rainbow: Arco Iris}
	...
	\subsection{ATAM para Flexibilizar la Auto Reparación}
		...
		\subsubsection{La Cuantificación de la Respuesta en la Auto Reparación}
			...
			de la arquitectura descripto en el lenguaje Acme, perdiendo toda correlación con su estímulo desencadenador. Veamos			un ejemplo extraído de Znn de cómo se implementan dichas restricciones al utilizar Rainbow:
			\begin{Verbatim}[gobble=4]
	...
	\subsection{Detección: cuando activar el mecanismo de auto reparación}
		...
		tipo de restricción permite fácilmente incorporar una nueva implementación implementando una \emph{interface} muy		sencilla (ver el apéndice \ref{sec:numericBinaryRelationalConstraintCode}). Estas \emph{constraints} serán de vital		importancia a la hora de activar el mecanismo de auto reparación.		
		...
		correspondientes a cada estímulo, así, ante la invocación de un estímulo en el sistema, Arco Iris solo deberá verificar
		que se cumplan los escenarios relacionados con dicho estímulo, acotando así de manera sustancial la cantidad de chequeos
		a realizar. Recordar que en Rainbow, ante cualquier modificación en el estado de la arquitectura, es necesario verificar
		todas las restricciones definidas para la auto reparación.
	...
	\label{sec:strategySelection}
		...
		configuración de Arco Iris también permite la posibilidad de considerar todas las estrategias existentes. Rainbow
		descartará las estrategias cuyas condiciones no se satisfagan, están condiciones generalmente se corresponden con la
		restricción que ha dejado de cumplirse, por lo cual su lógica se encuentra duplicada en el modelado de la arquitectura
		y en la condición de la estrategia, consideremos la siguiente estrategia tomada de Znn a modo de ejemplo:
		...
		Ahora bien, una vez determinado el subconjunto de estrategias a considerar, es necesario asignarle un peso a cada
		...
		\subsection{Selección de estrategia según Rainbow}
	
			Una vez obtenido el subconjunto de estrategias aplicables, Rainbow se encargará de asignarle un
		
		Calculate scores of the enabled strategies
	 *     = this involves evaluating the meta-information of the tactics
	 *       in each strategy
	 
			\todo{Mencionar como se usan los pesos asignados a los concerns en los scenarios de Rainbow (utilities.yml)}
		
		\subsection{Selección de estrategia según Arco Iris}
