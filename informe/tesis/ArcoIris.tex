\section{Extensión a Rainbow: Arco Iris}
	...
	\label{sec:strategySelection}
		...
		reduce ampliamente el universo de posibilidades. Recordemos que la configuración de Arco Iris también permite la
		posibilidad de considerar todas las estrategias. Rainbow descartará las estrategias cuyas condiciones no se
		satisfagan, están condiciones generalmente se corresponden con la restricción que ha dejado de cumplirse, por lo cual
		su lógica se encuentra duplicada en el modelado de la arquitectura y en la condición de la estrategia, consideremos la
		siguiente estrategia tomada de Znn a modo de ejemplo:
		...
		\label{sec:systemUtility}
			...
			En el caso de Rainbow esta medida permitirá dilucidar como impactará una determinada estrategia sobre el sistema,
			para lo cual cada estrategia deberá explicitar los \emph{concerns} sobre los que impacta y en qué medida lo hace. Por
			ejemplo, en el caso de bajar un servidor en Znn se establece el siguiente impacto:
			...
			Esto representa que la performance se verá degrada en 144 milisegundos, mientras que el costo se reduce en 1.
			
			\todo{CONTINUAR DE ACA!}

		\subsubsection{Peso de un escenario}
			\label{sec:scenarioWeight}

			\todo{Explicar Peso de un escenario}
