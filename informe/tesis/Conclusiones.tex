\section{Conclusiones}
	...
	\subsection{Arco Iris comparado con Rainbow}
		...
		\noindent En la presente sección se repasarán las ventajas que posee Arco Iris por sobre Rainbow. Se profundizará en
		los puntos básicos, sumarizados en la figura \ref{fig:arcoIrisVsRainbow}.
			...
			\begin{figure}[ht]
				\begin{center}
		 			...
		 			\begin{tabularx}{\textwidth}{|X|X|X|X|}
		  			...
		  			Información sobre restricciones & Guardadas en el modelo de la arquitectura (en \mbox{Acme}) & Mediante
		  			escenarios de QAW editados por \emph{stakeholders} & Fácil y transparente agregado y edición de restricciones.\\
		  			...
		  			dicha estrategia sea más adecuada para la situación actual del sistema.
		 			\end{tabularx}
		 			}
				\end{center}
...
% 		\end{center}
		...
		\subsubsection{Información sobre restricciones}
			...
			En Arco Iris, los \emph{stakeholders} usan una interfaz visual intuitiva y fácil de utilizar (ver sección
			\ref{sec:arcoIrisUI}) para expresar restricciones del sistema en formato de escenarios de QAW; los cuales a su vez
			fueron pensados originalmente para facilitar la participación de personas con roles funcionales.
		...
		\subsubsection{Decisiones sobre que reparaciones realizar}
			...
			definidas en dicho subconjunto de escenarios. Este enfoque, además de ser más eficiente, es considerablemente más
			sencillo de configurar y tiene más sentido a nivel funcional, ya que en el caso de Rainbow no parece haber una manera
			sencilla de intuir de antemano qué estrategia eligiría en cada caso, hecho que complica en análisis de las
	...
	\subsection{Compatibilidad hacia atrás: una empresa sin sentido}
		...
		\begin{itemize}
			...
			característica de Rainbow, ya el foco absoluto de la auto reparación no estaría puesto en los Escenarios de atributos
			de calidad sino que paralelamente existirían dos flujos distintos de auto reparación disputando entre sí. Esto
			llevaría a un comportamiento excesivamente complejo y poco predecible, lo cual resulta poco deseable para un
	...
	\subsection{Aplicabilidad en sistemas reales}
		...
		\begin{itemize}
			...
			arquitectura, tal como es requerido por Rainbow o Arco Iris. Esto complica la adopción de \emph{frameworks}
			que centren la auto reparación de un sistema en el modelo de su arquitectura. Sin embargo, como hemos mencionado en
			la sección \ref{sec:ausenciaObsolescenciaModeloArquitectura}, existen líneas de investigación intentando atacar este
			problema, derivando el modelo de la arquitectura de un sistema a partir de información obtenida durante su ejecución,
	...
	\subsection{Soluciones Dinámicas a Entornos de Ejecución Cambiantes}

		Arco Iris, la extensión a Raibow desarrollada en este trabajo, aporta un ladrillo más a la construcción de una
		solución de auto reparación para sistemas de \emph{software} que permita que el sistema, en lugar de
		\textbf{implementar soluciones} para satisfacer requerimientos de atributos de calidad, directamente \textbf{conozca
		dichos requerimientos}, disponiendo así de un \emph{framework} genérico y personalizable que provea diversas
		soluciones que se \textbf{adaptan dinámicamente} al cambio de requerimientos producto de los \textbf{cambios en el
		entorno} de ejecución del sistema.

		Este dinamismo en la auto reparación de sistemas, junto a la flexibilidad y facilidad de configuración agregadas al
		modelo de selección de soluciones, hacen de Arco Iris una opción interesante a ser considerada para su
		estudio y extensión, a fin de \textbf{continuar avanzando en la flexibilización de herramientas de auto reparación}
		para sistemas de \emph{software} y consecuentemente, tender a la adopción progresiva de este tipo de herramientas por
