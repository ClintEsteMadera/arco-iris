...
\section{Implementación de \texttt{NumericBinaryRelationalConstraint}}
...
\section{Configuración de Arco Iris en Representación XML}
...
\section{Implementación de \texttt{FileSelgHealingConfigurationDao}}
\label{sec:fileSelfHealingConfigDao}
...
\label{sec:instalacionYEjecucionArcoIris}
	...
	\label{sec:prerrequisitos}
		...
		\subsubsection{Requisitos para la Ejecución de las Aplicaciones}
			...
			en la computadora dónde se desee ejecutar estas aplicaciones, la cual se encuentra disponible para la gran
		...
		\subsubsection{Requisitos para la Inspección y Edición del Código Fuente}
			...
			\begin{itemize}
				\item \textbf{Maven plugin}: El plugin para Eclipse de Maven, el cuál puede ser obtenido utilizando el
 		...
 		\begin{center}
 			\begin{tabularx}{\textwidth}{p{2cm}X}
	  		...
	  		\begin{tabularx}{\textwidth}{p{2cm}X}
	  			...
	  			este trabajo: Arco Iris, Arco Iris UI, y algunos módulos de Rainbow que fueron ligeramente modificados. 
	  		\end{tabularx}
			\begin{tabularx}{\textwidth}{p{2cm}X}
		...
		\end{figure}
		 ...
		 \begin{center}
			...
			\begin{tabularx}{\textwidth}{p{3cm}X}
				...
				características comunes a los proyectos relacionados con Arco Iris: \verb@arco-iris-core@, y \verb@arco-iris-ui@.
	  		\end{tabularx}
			\begin{tabularx}{\textwidth}{p{3cm}X}
				\textbf{org.sa.rainbow.*} & Aquellos proyectos Java cuyo nombre comienza con el prefijo \verb@org.sa.rainbow@ son
				básicamente \textbf{copias} de submódulos de Rainbow que fueron modificados ligeramente principalmente a fines de
				mejorar el \emph{logging} de los componentes \emph{core} de Rainbow. A fines prácticos, se podría prescindir de
				estos proyectos y Arco Iris debería seguir funcionando de la misma manera, aunque el \emph{log} de la aplicación
				sería bastante más verboso.
	  		\end{tabularx}
 		\end{center}

	\subsection{Compilación de Arco Iris y Arco Iris UI}

		A fines de compilar y construir los artefactos \verb@.jar@ que luego serán utilizados para ejecutar Arco Iris y Arco
		Iris UI, es necesario:
		\begin{itemize}
			\item construir el \verb@master-pom@, el cual luego será utilizado tanto por Arco Iris como por Arco Iris UI.
			\item instalar las dependencias de \verb@arco-iris-core@ en el repositorio local de Maven.
			\item construir \verb@arco-iris-core@ y \verb@arco-iris-ui@ de una sola vez, utilizando el \emph{reactor-pom} ubicado
			dentro del directorio \verb@arco-iris@.
		\end{itemize}
		...
		\subsubsection{Instalación Manual de Dependencias del Core de Arco Iris}
			...
			Afortunadamente, Maven ofrece una forma sencilla de instalar manualmente artefactos en el repositorio local. Los
			comandos a ejecutar están especificados en el \emph{script} (el cuál debería funcionar tanto en Linux como en
			Windows) ubicado en la siguiente ruta: \verb@projects/arco-iris/arco-iris-core/lib/install_missing_artifacts.cmd@.
			
			El mismo procedimiento debe ser reiterado para aquellas dependencias de \verb@arco-iris-ui@ que no se encuentran en
			repositorios Maven públicos. Para ello, es necesario correr el \emph{script}
			\verb@projects/arco-iris/arco-iris-ui/lib/install_missing_artifacts.cmd@.
			
			Cabe mencionar que para los 2 proyectos Java aquí presentados (\verb@arco-iris-core@ y \verb@arco-iris-ui@) se
			incluye un directorio \verb@lib@ con las dependencias de cada proyecto, de modo que cada uno pueda compilar y correr
			sin que el usuario necesite disponer de una conexión a internet.

		\subsubsection{Construcción de los Proyectos con Maven}
		
			Generar los binarios de las dos aplicaciones anteriormente mencionadas, es tan simple como ejecutar los siguientes
			dos comandos:
			...
			\begin{enumerate}
				\item el archivo \verb@arco-iris-ui-0.0.1-SNAPSHOT.jar@ con los binarios de \textbf{Arco Iris UI}, compilados
				utilizando las librerías nativas del sistema operativo subyacente (Windows y Linux están actualmente soportados,
				para sus versiones de 32 y 64 bits)
				\item el archivo \verb@arco-iris-ui-0.0.1-SNAPSHOT-distribution.zip@ que contiene el \verb@.jar@ anteriormente
				mencionado de \textbf{Arco Iris UI} junto con un directorio \verb@lib@ que posee todas las dependencias de dicha
				aplicación.
...
\label{sec:logCasoPruebaArcoIris}
	...
	A continuación, se incluye un extracto significativo del \emph{log} generado por Arco Iris para el caso de prueba
	número 2, presentado anteriormente en la sección \ref{sec:testCase2}. Dada la longitud del \emph{log} original, el
	mismo ha sido recortado a fin de mostrar las partes más importantes del procesamiento que efectúa Arco Iris para el
	caso de prueba de referencia.
	
	Observese que Arco Iris al iniciar, toma desde el directorio\\
	\verb@/home/user/arco-iris/arco-iris-core/configs/TestCase2@ toda la configuración requerida, tanto por Rainbow como
	por Arco Iris (archivo \verb@.sim@ de simulación, tácticas y estrategias, el modelo de la arquitectura en Acme, el
	archivo XML de configuración de Arco Iris, etc.)
	
	Nótese que luego Arco Iris, de acuerdo a la configuración en el archivo de simulación, efectúa la modificación de
	varias propiedades del sistema de acuerdo a lo que allí se indica.
	
	Finalmente, para esta porción de \emph{log}, se buscan escenarios rotos, no encontrando ninguno en esta etapa de la
	ejecución.
	...
	Luego de un tiempo, se vuelve a ejecutar la rutina de búsqueda de escenarios que no se cumplen y se detecta que el
	escenario de tiempo de respuesta se encuentra ``roto'' (el tiempo de respuesta ha subido a 602,25 ms. cuando el umbral
	máximo era 600 ms.). Se evalúa la única estrategia existente para dicho escenario y se decide ejecutarla debido a que
	se estima que el tiempo de respuesta luego de la ejecución de dicha estrategia, será de 458,25 ms.
	
	Nótese que los resultados de ejecutar la táctica \verb@enlistServers@ (que agrega un servidor más al sistema),
	difícilmente sean instantáneos, es decir, con lo cual, junto a la ejecución de dicha táctica, se determina una demora
	(\emph{delay}) de 2,5 segundos (2529,55 ms.) para esperar a que los resultados de la táctica sean visibles en el
	sistema en \emph{runtime}.
	...
	Finalmente, luego de varios segundos de ejecución y habiendo esperado el tiempo prudencial para determinar si el haber
	levantado un servidor más en el sistema tuvo el efecto esperado o no, cerca del segundo 8 de ejecución (8012 ms.) se
	arriba a un estado dónde el sistema baja su tiempo de respuesta promedio a 583,73, ligeramente por debajo del umbral
	máximo: 600 ms. Así, el sistema decide que el resultado (\emph{outcome}) de la estrategia es exitoso y determina que en
	ese momento ya no hay más escenarios sin cumplirse.
	
	Alrededor de los 40 segundos (valor configurado en el archivo de simulación), la ejecución del sistema simulado termina
	y se indica a al tándem Rainbow/Arco Iris que termine también, finalizando así el ciclo de adaptación.
