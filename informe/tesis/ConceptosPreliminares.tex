	...
	\subsection{Rainbow}
		...
		\subsubsection{Introducción a Rainbow}
			...
			\subsubsection{Arquitectura de Rainbow}
				...
				\begin{itemize}
					\item \textbf{infraestructura de adaptación} la cual se divide a su vez en capas que proveen funcionalidades
					comunes a distintos sistemas auto adaptables logrando por lo tanto el objetivo de disponer de componentes
					reutilizables, a saber:
					\begin{enumerate}
						...
						recurso, entre otros criterios. Finalmente, un mecanismo denominado \emph{Effector} lleva a cabo la modificación
						propiamente dicha del sistema.
						\item \textbf{Capa de Arquitectura}:\\
						En esta capa, los denominados \emph{Gauges} agregan información provenientes de los \emph{Probes} y actualizan
						las propiedades correspondientes en el modelo arquitectural del sistema, el cual es manejado y accedido mediante
						un componente denominado \emph{Model Manager}. El \emph{Constraint Evaluator} chequea el modelo periódicamente y
						dispara la adaptación en el caso que ocurra una violacin en alguna restricción impuesta sobre el modelo. En ese
						caso, el motor de adaptación (\emph{Adaptation Engine}) determina el curso de acción y lleva a cabo la adaptación
						necesaria.
						...
						mapeos que distintos componentes dentro de esta capa comparten, por ejemplo, información necesaria para traducir
						un elemento arquitectónico en una dirección IP o una operación a nivel modelo de la arquitectura en su
						correspondiente operación de \emph{runtime}. Por ejemplo:
					...
					\item \textbf{conocimiento específico del sistema sobre adaptación}:\\
					Agregar auto adaptación a un sistema de software utilizando la funcionalidad que la infraestructura provista por la
					infraestructura de adaptación requiere agregar conocimiento específico de la aplicación para poder ajustar dicha
					infraestructura a las necesidades puntuales de cada sistema. Este conocimiento incluye el modelo operacional del
					sistema, que define parámetros como tipos de componentes y propiedades, restricciones de comportamiento,
					...
					Típicamente este conocimiento no es reutilizable entre distintos sistemas.
				\end{itemize}
		...
		\subsubsection{Tácticas y Estrategias}
			...
			las condiciones de ejecución del mismo. En el libro \textbf{Software Arquitecture in Practice} \cite{BassClementz},
			...
			\todo{TURCO: REALMENTE ESTA ES LA DEFINICION? Necesitamos algo un poco mejor\ldots}
			Allí se definen las tácticas como una decisión de diseño que influye sobre la respuesta \todo{???? que es la
			respuesta de un QA?} de un atributo de calidad, mientras que definen a una estrategia como una colección de tácticas.
			La figura \ref{fig:tactics_control_to_response} muestra gráficamente la visión de las tácticas presentada por los
			autores.

			\todo{este gráfico queda colgadísimo así como está explicado hasta el momento el tema}
			\begin{figure}[h]
				\centering
			...
			\todo{El hablar continuamente del libro no es del nivel de una tesis. Se menciona una vez la bibliografía y luego se
			hace al texto propio.}
			...
			En el ejemplo anterior se puede observar una característica esencial de las estrategias: las mismas afectan
			(positiva y/o negativamente) a los atributos de calidad establecidos para un sistema y cuando una estrategia se
			diseña para mejorar un atributo de calidad en particular, se puede decir que los \emph{stakeholders} que definen
			dichos atributos de calidad obtienen cierto ``beneficio'' de las estrategias, el cual se suele denominar
			\emph{utilidad}.

			Cada estrategia provee un nivel específico de utilidad, pero en contrapartida presenta un costo en tiempo y, sobre
			...
			\todo{corregir esto: se mezclan conceptos de Atributo de Calidad y Concern.}
		...
		\subsubsection{Stitch}
			A fin de disponer de una forma suficientemente expresiva de expresar tácticas y estrategias, Rainbow incluye
			un lenguaje de propósito particular llamado \textbf{Stitch}, el cual permite plasmar el conocimiento rutinario de
			las personas sobre adaptación de sistemas de software. Tanto las tácticas como las estrategias serán descriptas
			utilizando Stitch. Ofrece modularidad con respecto a las dimensiones de los atributos de calidad y al dominio,
			permite especificar estrategias de múltiples pasos con condiciones de aplicabilidad y efectos deseados y provee	un
			mecanismo para extender las políticas de adaptación a dominios específicos de sistemas.

			\todo{Completar esto con el contenido de doc/rainbow/CompArch 6-09.pdf}
