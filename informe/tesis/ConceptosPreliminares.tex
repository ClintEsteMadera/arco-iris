\section{Conceptos Preliminares}
	...
	\subsection{Auto Reparación}
		...
		cuanto a disponibilidad se trata. Hoy en día los mismo operan en ambientes muy dinámicos, con requerimientos del
		usuario muy cambiantes y con la necesidad de operar prácticamente sin interrupción, resultando esto en un
		...
		errores en el sistema. Esto es lo que se conoce como \emph{self healing} o, en castellano, auto reparación.

		Si bien ya existen mecanismos para mitigar dichos problemas, ellos forman parte del lenguaje de programación
		utilizado, tales como tratamiento de excepciones, protocolos de tolerancia a fallos, etc. Pero estos mecanismos
		son comúnmente dependientes de cada aplicación y generalmente muy dependientes del código de la aplicación que se
		intenta adaptar. Así, hoy en día, la adaptación de sistemas de software es costosa de construir, dificil de modificar,
		poco reusable entre distintos sistemas y generalmente solo provee soluciones a fallos de manera puntual.

		En cuanto al estado del arte en materia de auto reparación (o de manera más genérica, sistemas autónomos) existen
		diversos enfoques tanto en el ámbito académico como en la industria del software. 

		Dentro del ámbito de la industria, el concepto de sistemas autónomos se encuentra - al igual que en el plano académico
		- ampliamente difundido. Sin dudas el enfoque de IBM, denominado ``Autonomic Computing''\cite{IBM-AC} es el más
		completo, apalancado por un gran grupo de investigación y abarcando el problema desde distintos aspectos. También se
		destaca la iniciativa de Microsoft denominada ``BNla'', y no tanto la iniciativa de Sun (``Bla''), por estar más
		ligada a adaptar el enfoque al dominio de los sistemas operativos.

		Por otro lado, existen distintos investigadores ligados a prestigiosas instituciones académicas abocados al estudio de
		la autonomía de los sistemas. Uno de los primeros investigadores en acuñar el término ``Self Healing'' fue el Dr. David
		Garlan, de la Universidad de Carnegie Mellon, quien formó un grupo de investigación que dedicó años a estudiar el tema
		dentro del marco del proyecto Able.
	...
	\subsection{Lenguajes de Descripción de Arquitecturas}
		...
		ha motivado el avance de los ADLs (Architecture Description Language).\cite{ACME}

		La descripción de arquitecturas de sistemas basada en ADLs ha avanzado muchísimo en las últimas 2 décadas, al punto de
		que ya permiten definir una base formal para su descripción y análisis.

	\subsubsection{Acme}
	\label{sec:acme}

		Acme es uno de los ADLs más reconocidos y utilizados, ha sido desarrollado en la Carnegie Mellon University, más
		precisamente por el proyecto ABLE \footnote{El proyecto ABLE (\emph{``Architecture Based Languages and
		Environments''}) de la Universidad de Carnegie Mellon lleva a cabo investigaciones que conducen a una base de
		ingeniería para la arquitectura de software. Para más información, visitar \url{http://www.cs.cmu.edu/~able}},
	...
	\subsection{Rainbow}
		...
		\subsubsection{Introducción a Rainbow}
			...
			\begin{itemize}
				...
				\item Resolución: la habilidad de determinar cuando las propiedades observadas del sistema violen los supuestos
		...
		\label{sec:exponentialAverage}
			...
			El nombre factor de suavizado no es un nombre muy adecuado, ya que en realidad a mayor $\alpha$ menor será el suavizado
			del promedio, mientras que valores cercanos a cero harán que el promedio se vea menos afectado por el nuevo valor
			observado. No existe un procedimiento formal para determinar el valor de $\alpha$, en el caso de Rainbow el valor
			escogido para las pruebas realizadas ha sido 0.3.
