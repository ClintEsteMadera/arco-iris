\section{Conceptos Preliminares}
	...
	\subsection{Escenarios de Atributos de Calidad}
		...
		desarrolladores, usuarios, \emph{sponsors}, etc). Estos escenarios pueden luego ser utilizados para analizar y definir
		la arquitectura del sistema e identificar \emph{concerns} y posibles estrategias para atacar problemas.
		...
		\subsubsection{QAW}
			...
			seguridad, etc, que son derivados de objetivos del negocio. QAW no asume la existencia de una arquitectura del
			software, sino que fue desarrollado como consecuencia de la necesidad de \emph{stakeholders} y arquitectos de un
	...
	\label{sec:atam}
		...
		\subsubsection{Terminología necesaria}
			...
			\begin{itemize}
				...
				para poder alcanzar un atributo de calidad determinado.
				\item \textbf{Punto de \emph{tradeoff}}: propiedad que afecta y es un punto sensible para más de un atributo de
				calidad.
				\item \textbf{\emph{Non risks}} son buenas decisiones de arquitectura que normalmente están implícitas en la
				arquitectura.
			\end{itemize}

			\todo{SANTIAGO: Nos darías algún ejemplo concreto de sensitivity point y tradeoff?}
		...
		\subsubsection{Analizando arquitecturas con ATAM}
			...
			\textbf{antes de que existiese la arquitectura} sobre la cual ATAM trabajaría.
	
		\subsubsection{Pasos del método ATAM}

			\todo{SANTIAGO: Observamos que ATAM es casi igual a QAW (lo incluye)\ldots vale la pena mencionar QAW entonces?}

			A continuación se describen los pasos del método ATAM:
	
			\noindent \emph{Presentación}

			\begin{enumerate}
				\item[1.] \textbf{Presentar ATAM.} El método es descrito a los \emph{stakeholders}.
				\item[2.] \textbf{Presentar las metas del negocio.} El \emph{project manager} describe los objetivos del
				negocio que motivan el desarrollo.
				\item[3.] \textbf{Presentar la arquitectura.} El equipo de arquitectos presenta la arquitectura propuesta,
				haciendo foco en cómo la misma satisface las metas del negocio.
			\end{enumerate}
			
			\noindent \emph{Investigación y Análisis}
			
			\begin{enumerate}
				\item[4.] \textbf{Identificar enfoques arquitectónicos.} Los enfoques arquitectónicos son identificados
				por el equipo de arquitectura, pero no analizados.
				\item[5.] \textbf{Generar el árbol de utilidad.} Se recaban los atributos de calidad que agregan utilidad al
				sistema y se los especifica en forma de escenarios priorizados.
				\item[6.] \textbf{Analizar enfoques arquitectónicos.} Se delinean enfoques arquitectónicos para los escenarios de
				mayor prioridad recabados en 5. Durante este paso, se identifican riesgos arquitectónicos, puntos sensibles y puntos
				de \emph{tradeoff}.
			\end{enumerate}

			\noindent \emph{\emph{Testing}}

			\begin{enumerate}
				\item[7.] \textbf{\emph{Brainstorming} y priorización de escenarios.} Basado en los escenarios del árbol de
				utilidad, se genera un conjunto mayor de escenarios más específicos. Estos nuevos escenarios son priorizados vía un
				proceso de votación.
				\item[8.] \textbf{Analizar enfoques arquitectónicos.} Se reitera el paso 6 pero con los escenarios más
				prioritarios encontrados en el paso 7, con el objetivo de encontrar nuevos riesgos, puntos sensibles y tradeoffs.
			\end{enumerate}

			\noindent \emph{\emph{Reporting}}

			\begin{enumerate}
				\item[9.] \textbf{Presentar resultados.} Basado en toda la información recolectada durante el proceso (escenarios,
			...
			\begin{figure}[h]
				\centering
					\includegraphics{images/ATAM_steps.png}
