\section{Conceptos Preliminares}
	...
	\subsection{Ingeniería de Requerimientos}
		...
		\subsubsection{Requerimientos No Funcionales o ``Restricciones''}

			En el desarrollo de sistemas críticos, no es suficiente el satisfacer sólamente \textbf{requerimientos funcionales}
			(i.e. aquello que el software deber hacer desde el punto de vista del negocio al que da soporte). Este tipo de
			sistemas generalmente deben satisfacer otro tipo de \textbf{requerimientos no funcionales}, también llamados
			\textbf{Restricciones}, los cuales especifican criterios a ser usados para juzgar la operación de un sistema en lugar
			de describir funcionalidad específica para el mismo.

			\begin{figure}[h]
				\centering
					\includegraphics[width=0.5\textwidth]{images/requerimientos.png}
				\caption{Ubicación de los Atributos de Calidad en los requerimientos}
	...
	\label{sec:atam}
		...
		\subsubsection{Analizando arquitecturas con ATAM}
			...
			método ATAM que detallaremos a continuación, no será difícil para el lector inferir que en realidad ATAM incluye a
			QAW.
