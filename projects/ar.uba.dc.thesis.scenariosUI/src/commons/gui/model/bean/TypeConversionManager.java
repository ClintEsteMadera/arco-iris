 ...
 * <br>
 * Esta clase permite registrar conversores de tipos que son utilizados por otros componentes del
 * framework (especialmente por {@link commons.gui.model.bean.BeanAttribute}). De esta forma es
 * posible, por ejemplo, manipular objetos de tipos "no-standard" (<code>NumeroDeDocumento</code>,<code>Cuit</code>,etc)
 * como si fueran objetos "standard" (int,String) facilitando el data-binding.
 * @author ppastorino
 */
public class TypeConversionManager {

	/**
	 * Registra un conversor de tipos. A los efectos del data-binding las propiedades de los beans
	 * que sean del tipo especificado como <code>sourceClass</code> se van a comportar como si
	 * fueran de tipo <code>targetClass</code>
	 * @param sourceClass
	 *            Clase original. Si se quiere manipular la propiedad de un bean que tiene un tipo
	 *            no standard, debe especificarse el tipo no standard en este parámetro. Por
	 *            ejemplo: <code>NumeroDeDocumento.class</code>
	 * @param targetClass
	 *            Clase "destino" a la cual se quiere convertir. Si se quiere manipular la propiedad
	 *            de un bean que tiene un tipo no standard, debe especificarse el standard en este
	 *            parámetro. Por ejemplo: <code>Integer.class</code>
	 * @param conversor
	 *            Conversor que convierte de la clase original a la clase destino mediante el método
	 *            {@link sba.ui.util.ValueConversor#convertTo(java.lang.Object)} y de la clase destino
	 *            a la clase original mediante el método
	 *            {@link sba.ui.util.ValueConversor#convertTo(java.lang.Object)}
	 */
	static public void addConvertibleType(Class sourceClass, Class targetClass,
			ValueConversor conversor) {
