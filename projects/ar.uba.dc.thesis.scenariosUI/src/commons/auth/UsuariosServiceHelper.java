...
public class UsuariosServiceHelper implements AuthenticationHelper {
	...
	public boolean tiempoRemanenteExpirado() {
		...
		try {
			...
			log.error("La invocación al servicio de Usuarios para pedir el tiempo remanente del usuario " + usuario
					+ " produjo una excepción.", ex);
			throw new ApplicationException(
					"No se pudo consultar el tiempo remanente para el cambio de password del usuario " + usuario
							+ ". Contacte al administrador.", ex);
		}
	}
	...
	public List<String> obtenerRolesDelUsuario() {
		...
		try {
			DataIterator rolesIterator = (DataIterator) getUsuariosService().obtenerRolesDelUsuario();
			RolTO currentRolTO;
			while (rolesIterator.hasNext()) {
				currentRolTO = (RolTO) rolesIterator.next();
				roles.add(currentRolTO.getNombre().toUpperCase());
			}
			this.setCredentials(SessionHelper.nombreDeUsuarioConectado(), SessionHelper.passwordDeUsuarioConectado(),
					roles);
		} catch (NamingException ex) {
			throw new AuthenticationException(
					"No se pudieron obtener los roles para el usuario conectado. Contacte al administrador.", ex);
		} catch (Exception ex) {
			throw new ApplicationException(ex.getMessage(), ex);
		}
		return roles;
	...
	public void setCredentials(String username, String password, List<String> roles) throws NamingException {
		...
		env.setProperty("java.naming.security.credentials", SessionHelper.passwordDeUsuarioConectado());
