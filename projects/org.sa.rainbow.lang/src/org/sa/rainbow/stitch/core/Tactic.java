...
public class Tactic extends ScopedEntity implements IEvaluable {
	...
	public Object evaluate (Object[] argsIn) {
		...
		if (! checkCondition()) {  // don't continue execute?
			Tool.warn("Tactic condition of applicability NOT met!", stitch().stitchProblemHandler);
			// "unfreeze" model
		...
		for (Statement stmt : actions) {
			...
			if (stmt.hasError()) {
				m_hasError = true;
				Tool.error("Statement failed to evaluate! " + stmt.toString(),
		...
		if (Tool.logger().isInfoEnabled())
			Tool.logger().info(Tool.TAB + Tool.TAB + "Tactic expected effect: " + checkEffect());
