...
public class LiloScopeEstablisher extends AbstractLiloBehavior implements ILiloBehavior {
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.stitch.visitor.AbstractLiloBehavior#beginScript(org.sa.rainbow.stitch.core.IScope)
	 */
	@Override
	...
	public void endScript() {
		...
		if (script().tactics.size() == 0 && script().strategies.size() == 0) {
			//ALI: MODIFIED
			Tool.warn("A script with no tactic nor strategy defined is not very useful!", stitchProblemHandler());
		}
		popScope();
	}

	/* (non-Javadoc)
	 * @see org.sa.rainbow.stitch.core.AbstractLiloBehavior#createModule(antlr.collections.AST)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.stitch.core.AbstractLiloBehavior#createImport(antlr.collections.AST, antlr.collections.AST)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.stitch.visitor.AbstractLiloBehavior#addImportRename(antlr.collections.AST, antlr.collections.AST)
	 */
	@Override
	public void addImportRename(AST origAST, AST renAST) {
		if (script().renames.containsKey(renAST)) {  // uh oh, warn of duplicate name
			//ALI: MODIFIED
			Tool.error("Import rename used a duplicate name '" + renAST + "'!  Rename not valid.", stitchProblemHandler());
		} else {
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.stitch.core.AbstractLiloBehavior#createVar(antlr.collections.AST, antlr.collections.AST)
	 */
	@Override
	public void createVar (AST typeAST, AST nameAST) {
        ...
        if (! scope().addVar(var.name, var)) {
			...
			//ALI: Modified
        	Tool.error("Variable " + var.name + " redefined at line " + nameAST.getLine() + ":" + nameAST.getColumn(), stitchProblemHandler());
        }
	...
	public void beginExpression () {
		/*
		 * Expression IR-construction design intent:
		 * - Expressions are held as needed for evaluation to compute results.
		 * - Construct expression objects only for (a) top-level expression,
		 *   (b) complex expressions like quantified/method/list, and (c) any
		 *   expressions immediately "below" the complex expressions.
		 * - Use scoping while tree walking to maintain the additional info
		 *   needed to achieve the above construction.
		 *
		 * Algorithm:
		 * - At begin of an expression, check scope() to see if it's Expression
		 *   = if NOT an expression, construct an Expression and push as scope
		 *   = if it's an expression, see which kind it is
		 *     + if QUANTIFIED or LIST, construct an Expression and push as scope
		 *     + otherwise, just increment depth count and finish
		 * - At end of an expression, get the scope() Expression E
		 *   = if depth is non-zero, decrement depth count of E and finish
		 *   = if depth is ZERO, then
		 *     + store the AST in E 
		 *     + if parent scope is either (a) NOT an Expression or
		 *       (b) a complex Expression, also add E to parent' expression list
		 *     + pop scope
		 * - At begin of a "complex" expression,
		 *   = Construct an Expression CE marked with appropriate "complex" kind
		 *   = Push CE as scope
		 * - At end of a "complex" expression,
		 *   = Store AST and add this complex Expression to parent's list
		 */
	...
	public void endExpression () {
		...
		if (expr.subLevel > 0) {
			...
			if (addToParentList) {
				debug("=# add expression " + expr.getName() + " to parent " + expr.parent().getName());
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.stitch.core.AbstractLiloBehavior#endMethodCallExpression(antlr.collections.AST, antlr.collections.AST)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.stitch.core.AbstractLiloBehavior#endSetExpression(antlr.collections.AST)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.stitch.core.AbstractLiloBehavior#doExpression(antlr.collections.AST)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.stitch.core.AbstractLiloBehavior#doAssignExpression(antlr.collections.AST, antlr.collections.AST)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.stitch.core.AbstractLiloBehavior#doLogicalExpression(antlr.collections.AST)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.stitch.core.AbstractLiloBehavior#doRelationalExpression(antlr.collections.AST)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.stitch.core.AbstractLiloBehavior#doArithmeticExpression(antlr.collections.AST)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.stitch.core.AbstractLiloBehavior#doUnaryExpression(antlr.collections.AST)
	 */
	@Override
...
//		}
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.stitch.core.AbstractLiloBehavior#beginStatement(antlr.collections.AST)
	 */
	@Override
	public void beginStatement (AST stmtAST) {
		...
		if (stmt.type == Statement.Kind.IF
				|| stmt.type == Statement.Kind.FOR
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.stitch.visitor.AbstractLiloBehavior#markForCondition()
	 */
	@Override
	...
	public void markForEach() {
		...
		// designate the loop var by name, which should be the only var currently in scope
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.stitch.core.AbstractLiloBehavior#beginTactic(antlr.collections.AST)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.stitch.core.AbstractLiloBehavior#beginConditionBlock(antlr.collections.AST)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.stitch.core.AbstractLiloBehavior#beginActionBlock(antlr.collections.AST)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.stitch.core.AbstractLiloBehavior#beginEffectBlock(antlr.collections.AST)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.stitch.core.AbstractLiloBehavior#beginStrategy(antlr.collections.AST)
	 */
	@Override
	public void beginStrategy (AST nameAST) {
		...
		parentNode = curNode;  // root node serves as parent of all top-level strategy nodes
	...
	public void endBranching() {
		if (parentNode == null) {
			Tool.error("Error! The parent node is null when completing children branch!", stitchProblemHandler());
			return;
		}
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.stitch.visitor.AbstractLiloBehavior#beginStrategyNode(antlr.collections.AST)
	 */
	@Override
	...
	public void endStrategyNode() {
		if (curStrategy == null) {
			Tool.error("Unexpected error!  No current Strategy is set?!", stitchProblemHandler());
			return;
		}
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.stitch.visitor.AbstractLiloBehavior#doStrategyProbability(antlr.collections.AST, antlr.collections.AST, antlr.collections.AST)
	 */
	@Override
	public void doStrategyProbability(AST p1AST, AST p2AST, AST pLitAST) {
		if (curNode == null) {
			Tool.error("Expected to be processing condition part of a strategy tree node, but null curNode encountered!!", stitchProblemHandler());
			return;
		}
		...
		if (p1AST != null) {  // an identifier that needs later substitution
			...
			curNode.setHasProbability(true);
			curNode.setProbKey(null);  // just to be sure it'll be treated as literal
		...
		debug("* Probability gathered:  has it? " + curNode.hasProbability()
				+ ", key == " + curNode.getProbKey() + ", lit == " + curNode.getProbability());
	}

	/* (non-Javadoc)
	 * @see org.sa.rainbow.stitch.visitor.AbstractLiloBehavior#doStrategyCondition(org.sa.rainbow.stitch.core.Strategy.ConditionKind)
	 */
	@Override
	public void doStrategyCondition(ConditionKind type) {
		...
		switch (type) {
		case APPLICABILITY:  // in this case, should be the only expression in strategy scope
		...
		case FAILURE:
			if (parentNode == null || parentNode.getTactic() == null) {
				...
				Tool.error("The " + type.name() +
						" conditon is only applicable if there's a parent node that invokes a tactic!", stitchProblemHandler());
				return;
			} else {
				if (expr == null) {
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.stitch.visitor.AbstractLiloBehavior#doStrategyDuration()
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.stitch.visitor.AbstractLiloBehavior#beginReferencedTactic(antlr.collections.AST)
	 */
	@Override
	public void beginReferencedTactic(AST labelAST) {
		if (curNode == null) {
			Tool.error("Expected to be processing action part of a strategy tree node, but null curNode encountered!!", stitchProblemHandler());
			return;
		}
		...
		if (obj instanceof Tactic) {  // good, found tactic, store it
			...
			Tool.warn("Referenced tactic not yet defined? " + tacticName, stitchProblemHandler());
			// keep going though
		}
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.stitch.visitor.AbstractLiloBehavior#endReferencedTactic()
	 */
	@Override
	public void endReferencedTactic() {
		if (curNode == null) {
			Tool.error("Processing tactic reference in a strategy tree node, but null curNode encountered!!", stitchProblemHandler());
			return;
		}
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.stitch.visitor.AbstractLiloBehavior#doStrategyAction(org.sa.rainbow.stitch.core.Strategy.ActionKind)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.stitch.visitor.AbstractLiloBehavior#doStrategyLoop(antlr.collections.AST, antlr.collections.AST, antlr.collections.AST)
	 */
	@Override
	public void doStrategyLoop(AST vAST, AST iAST, AST labelAST) {
		...
		if (curStrategy.nodes.containsKey(doTarget)) {  // check if target has tactic action
			StrategyNode tgtNode = curStrategy.nodes.get(doTarget);
			if (tgtNode == null || tgtNode.getActionFlag() != ActionKind.TACTIC) {
				Tool.error("Invalid target node... target node have a Tactic action!", stitchProblemHandler());
				return;
			}
		} else {
			Tool.error("Do target does not refer to an existing label!", stitchProblemHandler());
			return;
		}
		// store target
		...
		if (vAST == null && iAST != null) {  // literal
			...
			if (vObj instanceof Var) {  // check to make sure type is correct
				...
				if (val instanceof Integer) {
					...
					Tool.error("Mismatched type in loop max count:  " + v.getType() + " instead of expected int", stitchProblemHandler());
					return;
				}
			} else {  // not expected var
				Tool.error("Identifier for loop max count should refer to a declared variable!", stitchProblemHandler());
				return;
			}
		} else {  // warning, neither a literal nor an identifier provided
			curNode.setNumDoTrials(StrategyNode.DEFAULT_LOOP_MAX);
			Tool.warn("No loop max count provided for DO loop, using default of " + StrategyNode.DEFAULT_LOOP_MAX, stitchProblemHandler());
	...
	private Expression doBeginComplexExpr (String name, Expression.Kind kind, boolean distinctScope) {
	...
	private Expression doEndComplexExpr () {
		...
		if (expr.parent() != null) {
			...
			Tool.error("Parent expression of complex expression is NULL?!", stitchProblemHandler());
		}
		popScope();
	...
	/**
	 * Resolve all the imports that have been created, by iterating through
	 * the list and seeking out the import targets.  For each import, depennding
	 * on the type, resolution occurs differently.  Each import brings in a
	 * namespace to which references can later go to resolve references.
	 * The different types of import causes the target to be treated differently:
	 * <br><br>
	 * <ul>
	 *  <li> <code>Import.LIB</code>:  another script to pull in
	 *  <li> <code>Import.MODEL</code>:  an architectural model file (default: Acme)
	 *  <li> <code>Import.OP</code>:  an operator file, could be style operator
	 *    or utility operators like the system Query interface
	 * </ul> 
	 */
		...
		for (Import imp : m_stitch.script.imports) {
			if (imp.type == Import.Kind.LIB) {
				...
				try {
					...
					if (stitch == null) {  // imported script file not previously parsed
					...
					//ALI: Modified
					Tool.error("Cannot find library file '" + imp.path + "' to import!", e, stitchProblemHandler());
					e.printStackTrace();
				}
			} else if (imp.type == Import.Kind.MODEL) {
				if (Ohana.instance().modelRepository() != null) {
					// Assume Acme Model for now
					try {
						Object o = Ohana.instance().modelRepository().getModelForResource(determinePath(imp.path).getCanonicalPath());
						if (o instanceof IAcmeModel) {
							IAcmeModel model = (IAcmeModel )o;
							m_stitch.script.models.add(model);
						} else {
							Tool.warn("No AcmeModel loaded, perhaps a dummy repository is used?!", stitchProblemHandler());
						}
					} catch (IOException e) {
						//ALI: Modified
						Tool.error("Unable to load model file " + imp.path, e, stitchProblemHandler());
						e.printStackTrace();
					}
				} else {
					Tool.error("Model repository not set yet! Unable to load model file " + imp.path, stitchProblemHandler());
				}
				...
				if (imp.path.endsWith(".*")) {  // yes, list classes in package
					...
					try {
						...
						Tool.warn("Package name in OP import appears invalid.", e, stitchProblemHandler());
						continue;
					}
				} else {
					...
					try {
						...
						Tool.warn("Class name in OP import appears invalid.", e, stitchProblemHandler());
						continue;
					}
				}
