...
/**
 * The Oracle class is a singleton class that coordinates the active components
 * of Rainbow and provides a control GUI.
 * 
...
public class Oracle implements IDisposable {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < numTrials; i++) {
			...
			instance().m_logger.info("|  Trial run " + (i + 1) + " of "
					+ numTrials + "...");
			instance().m_logger.info("\\========================\\");
			Util.pause(IRainbowRunnable.LONG_SLEEP_TIME); // wait a second for
			// GUI to start
			instance().run();
	...
	private Oracle() {
		...
		Rainbow.setIsMaster(true); // declare this to be the master Rainbow
		// component
		ARainbowLoggerFactory.reset(); // activate the relevant logger factory
		Rainbow.instance().serviceManager(); // activate the
	...
	public IRainbowRunnable modelManager() {
		if ((m_modelmgr == null || m_modelmgr.isDisposed())
				&& !Rainbow.shouldTerminate()) {
	...
	public IRainbowRunnable archEvaluator() {
		if ((m_evaluator == null || m_evaluator.isDisposed())
				&& !Rainbow.shouldTerminate()) {
	...
	public IRainbowRunnable adaptationManager() {
		if ((m_adaptmgr == null || m_adaptmgr.isDisposed())
				&& !Rainbow.shouldTerminate()) {
	...
	public IRainbowRunnable strategyExecutor() {
		if ((m_executor == null || m_executor.isDisposed())
				&& !Rainbow.shouldTerminate()) {
	...
	/*
	 * public ILearner learner () { if (m_learner == null) { m_learner = new
	 * Learner((IRainbowLearner )adaptationEngine()); } return m_learner; }
	 */

	/**
	 * Check to make sure all threads are terminated. Used to determine whether
	 * to contineu next trial run.
	 */
	public boolean allTerminated() {
		return targetSystem().isTerminated() && modelManager().isTerminated()
				&& archEvaluator().isTerminated()
	...
	private void run() {
		...
		IRainbowEventListener healthListener = healthListener(Util
				.genID(getClass()));
		Rainbow.eventService().listen(IEventService.TOPIC_RAINBOW_HEALTH,
				healthListener);
		// - initialize target system, but don't start yet
		TargetSystem sys = targetSystem();
		if (sys instanceof SimulationRunner) {
			((SimulationRunner) sys).setSimFile(Rainbow
					.property(Rainbow.PROPKEY_SIM_PATH));
		}
		...
		Util.pause(IRainbowRunnable.LONG_SLEEP_TIME * 1000);

		// Oracle monitoring loop, checking for Rainbow termination conditions
		while (!(Rainbow.shouldTerminate() && allTerminated() && Rainbow
				.eventService().readyForTermination())) {
			Util.pause(IRainbowRunnable.LONG_SLEEP_TIME); // sleep a bit and
			// check termination
			if (Rainbow.shouldTerminate() && !Ohana.isDisposed()) {
				// Stop the Stitch backend first, so that any strategy execution
				// eventually stops
	...
	private IRainbowEventListener healthListener(String id) {
		return new RainbowEventAdapter(id) {
			@Override
			@EventTypeFilter(eventTypes = { IProtocolAction.LOG_EVENT,
					IProtocolAction.LOG_TRANSLATOR })
			public void onMessage(IRainbowMessage msg) {
				IProtocolAction action = IProtocolAction.valueOf((String) msg
						.getProperty(IProtocolAction.ACTION));
				switch (action) {
				case LOG_EVENT:
					String txt = (String) msg
							.getProperty(IRainbowHealthProtocol.TEXT);
