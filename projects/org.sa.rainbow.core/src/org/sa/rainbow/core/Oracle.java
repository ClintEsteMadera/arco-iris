...
//	private ILearner m_learner = null;
	 ...
	 */
	private Oracle () {
		if (m_showGui) m_gui = new RainbowGUI();
...
/*	public ILearner learner () {
		if (m_learner == null) {
			m_learner = new Learner((IRainbowLearner )adaptationEngine());
		}
		return m_learner;
	}
*/
	...
	/**
	 * Check to make sure all threads are terminated.
	 * Used to determine whether to contineu next trial run.
	 */
	public boolean allTerminated () {
		return targetSystem().isTerminated()
			&& modelManager().isTerminated()
...
//			&& ((IRainbowRunnable )learner()).isTerminated()
	...
	public void writeManagerPanel (RainbowLogger logger, String text) {
		if (m_showGui) m_gui.writeText(RainbowGUI.ID_MODEL_MANAGER, text);
		logger.info(text);
	}

	public void writeEvaluatorPanelSL (RainbowLogger logger, String text) {
		if (m_showGui) m_gui.writeTextSL(RainbowGUI.ID_ARCH_EVALUATOR, text);
		logger.info(text);
	}
	public void writeEvaluatorPanel (RainbowLogger logger, String text) {
		if (m_showGui) m_gui.writeText(RainbowGUI.ID_ARCH_EVALUATOR, text);
		logger.info(text);
	}

	public void writeEnginePanel (RainbowLogger logger, String text) {
		if (m_showGui) m_gui.writeText(RainbowGUI.ID_ADAPTATION_MANAGER, text);
		logger.info(text);
	}

	public void writeExecutorPanel (RainbowLogger logger, String text) {
		if (m_showGui) m_gui.writeText(RainbowGUI.ID_EXECUTOR, text);
		logger.info(text);
	}

	public void writeSystemPanel (RainbowLogger logger, String text) {
		if (m_showGui) m_gui.writeText(RainbowGUI.ID_TARGET_SYSTEM, text);
		logger.info(text);
	}

	public void writeTranslatorPanel (RainbowLogger logger, String text) {
		if (m_showGui) m_gui.writeText(RainbowGUI.ID_TRANSLATOR, text);
		logger.info(text);
	}

	public void writeEventPanel (RainbowLogger logger, String text) {
		if (m_showGui) m_gui.writeText(RainbowGUI.ID_EVENT_BUSES, text);
		logger.info(text);
	}

	public void writeOraclePanel (RainbowLogger logger, String text) {
		if (m_showGui) m_gui.writeText(RainbowGUI.ID_ORACLE_MESSAGE, text);
		logger.info(text);
	}
