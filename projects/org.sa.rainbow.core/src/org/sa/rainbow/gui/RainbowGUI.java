...
public class RainbowGUI implements IDisposable {
	...
	public static final int TEXT_COLUMNS = 55;
	...
	/* pink */new Color(255, 145, 255), Color.RED,
	/* green */new Color(0, 255, 64), Color.CYAN,
	/* orange */new Color(255, 128, 64), Color.BLUE, Color.WHITE, Color.GRAY };
	private final int[] m_order = { 7, 2, 8, 3, 0, 1, 5, 4, 6 };
	...
	private void show() {
		...
		/*
		 * Proceed to set constraints in the following order: | 1 2 3 | | 4 5 6 | | 7 8 9 |
		 */
	...
	private JScrollPane createTextArea(int area) {
		...
		m_panes[area] = new JScrollPane(m_textAreas[area], JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
				JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		m_panes[area].setAutoscrolls(true);
		Border border = BorderFactory.createMatteBorder(PANEL_BORDER, PANEL_BORDER, PANEL_BORDER, PANEL_BORDER,
				m_colors[area]);
		...
		item.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String hostname = JOptionPane.showInputDialog(m_frame,
						"Please provide a hostname with a sleeping RainDropD");
				if (hostname != null && hostname.length() > 0) {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (hostname != null && hostname.length() > 0) {
					...
					if (rv == JFileChooser.APPROVE_OPTION) {
						...
						writeText(ID_ORACLE_MESSAGE, "Attempting remote update on " + hostname + " with "
