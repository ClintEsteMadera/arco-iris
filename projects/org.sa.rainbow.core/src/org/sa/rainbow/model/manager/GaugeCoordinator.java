...
/**
 * Coordinates the creation and deletion of, and coordination with, gauges.
 * Communication with the gauges occur across the Gauge Event Bus.  In addition,
 * every host is assumed to have a RainbowDelegate to act on behalf of this
 * coordinator on that remote machine.
 * <p>
 * At initiation, the coordinator reads a gauge specification file (most
 * likely separate from the Acme description of the target system) and causes
 * the deployment of the necessary gauges on the appropriate locations (hosts).
 * This coordinator keeps track of what gauges are created and active, and is
 * the contact point to obtain a reference to the IGauge.
 * <p>
 * Algorithm: <ol>
 ...
 * <li> Create all gauge instances
 * ({@link org.sa.rainbow.model.manager.ModelManager <code>ModelManager</code>}
 * takes care of this by invoking initGauges())
 ...
 */
public class GaugeCoordinator implements IDisposable, IGaugeConsumer {
	...
	public void dispose () {
		...
		for (GaugeInstanceDescription instSpec : new ArrayList<GaugeInstanceDescription>(m_id2Inst.values())) {
			synchronized (instSpec) {
				m_eventHandler.deleteGauge(instSpec.id(),
						new TypeNamePair(instSpec.gaugeType(), instSpec.gaugeName()),
						instSpec.modelDesc());
			}
		}
...
//		Util.logger().info("BEACON: Gauge ID[" + id + "]");
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.translator.gauges.IGaugeConsumer#onReportCreated(java.lang.String, org.sa.rainbow.util.TypeNamePair, org.sa.rainbow.util.TypeNamePair)
	 */
	public void onReportCreated (String id, TypeNamePair gaugeDesc, TypeNamePair modelDesc) {
		...
		if (instSpec != null) {
			...
			synchronized (instSpec) {
				...
				if (instSpec.gaugeType().equals(gaugeDesc.type())
						&& instSpec.modelDesc().equals(modelDesc)) {
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.translator.gauges.IGaugeConsumer#onReportConfigured(java.lang.String, org.sa.rainbow.util.TypeNamePair, org.sa.rainbow.util.TypeNamePair, java.util.List)
	 */
	public void onReportConfigured (String id, TypeNamePair gaugeDesc,
		...
		if (instSpec != null) {
			synchronized (instSpec) {
				...
				if (instSpec.gaugeName().equals(gaugeDesc.name())
						&& instSpec.modelDesc().equals(modelDesc)) {
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.translator.gauges.IGaugeConsumer#onReportDeleted(java.lang.String, org.sa.rainbow.util.TypeNamePair, org.sa.rainbow.util.TypeNamePair)
	 */
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.translator.gauges.IGaugeConsumer#onReportValue(java.lang.String, org.sa.rainbow.util.TypeNamePair, org.sa.rainbow.util.TypeNamePair, org.sa.rainbow.util.AttributeValueTriple)
	 */
		...
		if (instSpec != null) {
			synchronized (instSpec) {
				...
				if (instSpec.gaugeName().equals(gaugeDesc.name())
					...
					Oracle.instance().rainbowModel().updateProperty(value.type(), value.value());
					// log info
	...
	/* (non-Javadoc)
	 * @see org.sa.rainbow.translator.gauges.IGaugeConsumer#onReportMultipleValues(java.lang.String, org.sa.rainbow.util.TypeNamePair, org.sa.rainbow.util.TypeNamePair, java.util.List)
	 */
	...
	public void checkGauges () {
		for (GaugeInstanceDescription instSpec : Rainbow.instance().gaugeDesc().instDescList()) {
			...
			synchronized (instSpec) {
				if (m_id2Inst.containsKey(instSpec.id())
						&& instSpec.isAlive() && instSpec.beacon().isExpired()) {
					// TODO: action besides reporting when gauge dies...
					m_logger.error("GC: Gauge ID["
							+ instSpec.id() + "] seems to have died, removing!");
	...
	/**
	 * Method called via the ModelManager thread to issue create gauge events
	 * to the GaugeManagers to cause creation of Gauges.
	 * <p>
				...
				if (instSpec.notYetCreated()) {
					...
					if (location != null && m_sys.existsDelegateAtLocation(location)) {
						...
						m_eventHandler.createGauge(location,
								new TypeNamePair(instSpec.gaugeType(), instSpec.gaugeName()),
								instSpec.modelDesc(), instSpec.setupParams(), instSpec.mappings());
					}
	...
	/**
	 * Method called via the ModelManager thread to issue configure gauge events
	 * to, and thus configure, the Gauges that have been created.
	 * <p>
	 * Design Note:  This method is called by the ModelManager ONLY after all
	 * expected target locations have been created; after that, this method
	 * may be called whenever.  This design allows gauges to communicate with 
	 * probes across Delegate boundaries.
	 */
					...
					if (instSpec.id() != null) {
						...
						for (ValuePropertyMappingPair vp : instSpec.mappings()) {
							...
							if (val != null) {  // add to config list
								...
								try {
									String valStr = StandaloneLanguagePackHelper.defaultLanguageHelper().propertyValueToString(propVal.getValue(), null);
									AttributeValueTriple configTriple =
										new AttributeValueTriple(AttributeValueTriple.DEFAULT_TYPE,
												vp.valueName(), valStr);
						...
						m_eventHandler.configureGauge(instSpec.id(),
								new TypeNamePair(instSpec.gaugeType(), instSpec.gaugeName()),
								instSpec.modelDesc(), instSpec.configParams());
					}
	...
	private void log (String msg) {
		if (m_logger.isInfoEnabled()) m_logger.info("[GC] " + msg);
	}
