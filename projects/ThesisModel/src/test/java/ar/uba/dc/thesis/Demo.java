...
public class Demo {
	...
	public static void main(String[] args) {
		...
		addRepairStrategy(heavyLoadScenario, usabilityScenario, "disableDynamicContent");
	...
	private static void addRepairStrategy(SelfHealingScenario scenarioToRepair,
		...
		RepairStrategy strategy = RepairStrategyRepository.getRepairStrategy(repairStrategyName);

		SelfHealingTradeoff tradeoff = new SelfHealingTradeoff(Collections
				.singletonList(affectedScenario), Collections.<SelfHealingScenario> emptyList());
	...
	private static SelfHealingScenario getUsabilityScenario() {
		...
		return new SelfHealingScenario(scenarioName, Concern.RESPONSE_TIME, stimulusSource,
				pageRenderer.getOperation(RENDER_PAGE_OPERATION_NAME), environment, components,
