 ...
 * 
 */
public abstract class AbstractGUIUncaughtExceptionHandler implements
		UncaughtExceptionHandler {

	private static final String DEFAULT_ERROR_MESSAGE = "An error has occurred. See error log for more details.";

	private static final Log log = LogFactory
			.getLog(AbstractGUIUncaughtExceptionHandler.class);

	public AbstractGUIUncaughtExceptionHandler() {
		super();
	}

	public final synchronized void uncaughtException(Thread thread,
			Throwable throwable) {
	...
	protected void handleUnknownThrowable(Thread thread, Throwable throwable) {
		try {
			...
			final String localizedMessage = StringUtils.isBlank(throwable
					.getLocalizedMessage()) ? DEFAULT_ERROR_MESSAGE : throwable
					.getLocalizedMessage();
			...
			Runnable runnable = new Runnable() {
				public void run() {
					if (PageHelper.getMainWindow().getSystemConfiguration()
							.showStackTraceForErrors()) {
						InternalErrorDialog.openError(null, "Error",
								localizedMessage, dialogException);
					} else {
						MessageDialog
								.openError(null, "Error", localizedMessage);
					}
				}
			};
			...
			System.err.print((new StringBuilder())
					.append("Exception in thread \"").append(thread.getName())
					.append("\" ").toString());
