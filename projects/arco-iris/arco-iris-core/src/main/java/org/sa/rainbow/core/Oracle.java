...
import org.sa.rainbow.model.manager.ArcoIrisModelManager;
import org.sa.rainbow.monitor.SystemDelegate;
import org.sa.rainbow.monitor.TargetSystem;
import org.sa.rainbow.monitor.sim.ArcoIrisSimulationRunner;
...
/**
 * The Oracle class is a singleton class that coordinates the active components of Rainbow/Arco Iris and provides a
 * control GUI.
 */
public class Oracle implements IDisposable {
	public static final String NAME = "The Arco Iris Oracle";
	...
	private ArcoIrisModel m_model = null;
	...
	public Model rainbowModel() {
		if (m_model == null) {
			m_model = new ArcoIrisModel(selfHealingConfigurationManager());
	...
	public TargetSystem targetSystem() {
		if (m_system == null) {
			if (Rainbow.inSimulation()) {
				m_system = new ArcoIrisSimulationRunner();
			} else {
	...
	public IRainbowRunnable modelManager() {
		if ((m_modelmgr == null || m_modelmgr.isDisposed()) && !Rainbow.shouldTerminate()) {
			m_modelmgr = new ArcoIrisModelManager();
		}
		return m_modelmgr;
	}
	...
	public IRainbowRunnable adaptationManager() {
		if ((m_adaptmgr == null || m_adaptmgr.isDisposed()) && !Rainbow.shouldTerminate()) {
			m_adaptmgr = new ArcoIrisAdaptationManager(selfHealingConfigurationManager());
		}
		return m_adaptmgr;
	}
	...
	private void run() {
		...
		modelManager().start();
		new GraphicGenerator((ArcoIrisModel) this.rainbowModel()).start();
