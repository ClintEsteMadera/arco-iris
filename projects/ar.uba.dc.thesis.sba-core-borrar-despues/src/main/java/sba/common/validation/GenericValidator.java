...
public class GenericValidator implements Validator {
	...
	protected boolean validate(Object value, String property, ValidationHandler handler,
	...
	private Object invokeMethod(Object target, Method method) {
		try {
			...
			throw new RuntimeException("Error invocando el metodo " + method.getName()
					+ " de la clase " + method.getDeclaringClass().getName());
		}
	}
	...
	private void loadValidationInfo(Class clazz) {
		...
		try {
			...
			throw new RuntimeException("Error obteniendno información para la clase "
	...
	private String getValueDescription(PropertyDescriptor pd) {
		String valueDescription = AnnotationManager.getInstance().getValueDescription(
				pd.getReadMethod());

		if (valueDescription != null) {
			valueDescription = this.context.getEnumPropertiesDirectory().getEnum(valueDescription)
					.toString();
		}
	...
	private PropertyInfo loadPropertyInfo(Class targetClass, String propertyName) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (propertyDesc == null) {
				throw new IllegalArgumentException("No se encontró la propiedad '" + name
						+ "' para la clase " + targetClass.getName());
			}

			if (propertyDesc.getReadMethod() == null) {
				throw new IllegalArgumentException(
						"No se encontró el método de lectura para la propiedad '" + name
		...
		final PropertyInfo pi = new PropertyInfo(methods
				.toArray(new Method[methods.size()]), descriptions.toArray(new String[descriptions
				.size()]));
	...
	private static class ValidationInfo {
		...
		ValidationInfo(Method method, Pair<Annotation, ValidationHandler>[] annotations,
				String valueLocation, String valueDescription) {
			super();
			this.method = method;
	...
	private static class PropertyKey {
		...
		public boolean equals(Object obj) {
			...
			return k.propertyClass.equals(this.propertyClass) && 
				k.propertyName.equals(this.propertyName);
		}
