...
public interface ListValueModel<T> extends CollectionModel<T>, ValueModel<List<T>> {
	...
	/**
	 * Obtiene la lista subyacente. NOTA: las modificaciones que se hagan a la lista obtenida no se van a reflejar
	 * necesariamente en el modelo ni van a generar eventos. Si se quiere modificar el modelo, deben utilizarse los
	 * métodos <code>add, remove, clear, etc</code>
	 * 
	...
	/**
	 * Obtiene el índice de un elemento. Retorna el primer índice <code>i</code> para el cual se cumple
	 * <code>lista[i].equals(element)</code>
	 * 
	 * @param element
	...
	/**
	 * Obtiene el índice de un elemento, utilizando un comparador para la búsqueda. Retorna el primer índice
	 * <code>i</code> para el cual se cumple <code>comp(lista[i],value) == 0</code>
	 * 
	 * @param value
	 *            Elemento cuyo índice se quiere averiguar.
	 * @param comp
	 ...
	 * @return <code>true</code> en caso de que el elemento se haya eliminado o <code>false</code> si el elemento no
	 *         pertencecía a la lista.
	 */
