...
public class ValidateComponentHandler implements ValidationHandler<ValidateComponent> {
	...
	public void validate(Object value, ValidateComponent annotation, ValidationResult result, ValidationContext context) {
		if (value == null) {
			return;
		}
		...
		if (validator == null) {
			throw new IllegalArgumentException("No se definió un validador para la clase " + value.getClass().getName());
		}

		validator.validate(value, result);
	}

	private boolean evaluateCondition(Object value, ValidationCondition condition, ValidationContext context) {
	...
	private boolean isNotEmpty(Object value) {
		...
		try {
			...
			throw new IllegalArgumentException("La clase " + cl.getName() + " no tiene declarado el método '"
					+ IS_EMPTY_METHOD + "'", e);
		}
		...
		try {
			...
			if (!(result instanceof Boolean)) {
				throw new IllegalArgumentException("El método '" + IS_EMPTY_METHOD + "' en la clase " + cl.getName()
						+ " no retorna un booleano");
			}

			return !((Boolean) result).booleanValue();
		} catch (Exception e) {
			throw new IllegalArgumentException("Error invocando el método '" + IS_EMPTY_METHOD + "' en la clase "
					+ cl.getName() + ": " + e.getMessage(), e);
		}
